...
def _sales_trend(
	...
	*,
	...
	for source in dataset.get("trend") or []:
		key = _bucket_for_date(source.get("posting_date"), granularity)[0]
	...
	rows = [
		{
			"key": "available",
			"label": _("Available"),
			"value": summary.get("Available Items", 0),
			"drill_field": "stock_status",
			"drill_value": "Available",
		},
		{
			"key": "reorder_due",
			"label": _("Reorder Due"),
			"value": summary.get("Reorder Due", 0),
			"drill_field": "stock_status",
			"drill_value": "Reorder Due",
		},
		{
			"key": "out_of_stock",
			"label": _("Out of Stock"),
			"value": summary.get("Out of Stock", 0),
			"drill_field": "stock_status",
			"drill_value": "Out of Stock",
		},
		{
			"key": "negative",
			"label": _("Negative"),
			"value": summary.get("Negative Stock", 0),
			"drill_field": "stock_status",
			"drill_value": "Negative",
		},
		{
			"key": "fully_reserved",
			"label": _("Fully Reserved"),
			"value": summary.get("Fully Reserved", 0),
			"drill_field": "stock_status",
			"drill_value": "Fully Reserved",
		},
	]
	return {
