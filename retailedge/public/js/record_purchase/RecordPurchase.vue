			...
			<div v-else class="transaction-page-content">
				...
				<form v-if="!savedDocument || editingSavedDraft" class="edge-panel transaction-form" @submit.prevent="saveDraft">
					...
					<div class="field-grid">
						...
						<EdgeLinkField :modelValue="values.price_list" label="Buying Price List" placeholder="Select assigned Price List" description="The effective default follows Settings → Price List Governance. When switching is allowed, choose from Price Lists assigned to you for this Branch." :required="Boolean(formContext.pricing?.selection_required)" :disabled="editingSavedDraft || Boolean(formContext.pricing?.locked) || !Boolean(formContext.pricing?.can_select)" :searcher="searchPriceList" :context="searchContext" @update:modelValue="setPriceList" />
...
export default {
	...
	methods: {
		...
		setSupplier(next) { const value = next || ""; const changed = this.values.supplier !== value; this.values.supplier = value; this.pricingCache.clear(); if (changed) { this.values.items = this.values.items.map((row) => ({ ...row, rate: "" })); this.refreshPriceListContext().then(() => this.refreshAllItemPricing()).catch((error) => { this.saveError = errorMessage(error, "Unable to refresh Buying Price List."); }); } },
		async setBranch(next) { const branch = next || ""; this.values.branch = branch; this.values.warehouse = ""; this.pricingCache.clear(); if (!this.values.company) return; if (!branch) { try { await this.refreshPriceListContext(); this.refreshAllItemPricing(); } catch (error) { this.saveError = errorMessage(error, "Unable to refresh Buying Price List."); } return; } const token = ++this.cascadeToken; try { const r = await resolveBranchWarehouse({ company: this.values.company, branch, preference: "purchase" }); if (token !== this.cascadeToken) return; this.values.branch = r.branch || branch; this.values.warehouse = r.warehouse || ""; await this.refreshPriceListContext(); this.refreshAllItemPricing(); } catch (error) { if (token === this.cascadeToken) this.saveError = errorMessage(error, "Unable to resolve the Branch receiving stock location."); } },
		...
		pricingCacheKey(row) { return [this.values.company, this.values.branch, this.values.warehouse, this.values.supplier, this.values.price_list, this.values.posting_date, row.item_code, row.qty || 1].join("|"); },
		async loadItemPricing(index) { const row = this.values.items[index]; if (!row?.item_code || !this.values.supplier) return; const key = this.pricingCacheKey(row); const token = `${row.item_code}:${Date.now()}:${Math.random()}`; this.pricingTokens[index] = token; try { let result = this.pricingCache.get(key); if (!result) { result = await callMethod(PRICING_METHOD, { item_code: row.item_code, values: { company: this.values.company, branch: this.values.branch, warehouse: this.values.warehouse, supplier: this.values.supplier, price_list: this.values.price_list, posting_date: this.values.posting_date, qty: row.qty || 1 } }); this.pricingCache.set(key, result); } if (this.pricingTokens[index] !== token) return; if (result?.rate !== null && result?.rate !== undefined) this.values.items[index] = { ...this.values.items[index], rate: result.rate }; else this.saveError = `No buying price is configured for ${row.item_code}. Enter the agreed rate or configure Item Price.`; this.formContext.pricing = { ...(this.formContext.pricing || {}), ...(result || {}), price_list: result?.price_list || this.formContext.pricing?.price_list || "", source: result?.source || this.formContext.pricing?.source || "item_fallback" }; if (result?.price_list && (result?.locked || !this.values.price_list)) this.values.price_list = result.price_list; } catch (error) { if (this.pricingTokens[index] === token) this.saveError = errorMessage(error, `Unable to price ${row.item_code}.`); } },
