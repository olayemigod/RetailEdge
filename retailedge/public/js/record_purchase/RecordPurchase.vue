...
export default {
	...
	methods: {
		...
		setSupplier(next) { const changed = Boolean(this.values.supplier && this.values.supplier !== next); this.values.supplier = next || ""; this.pricingCache.clear(); if (changed) { this.values.items = this.values.items.map((row) => ({ ...row, rate: "" })); this.refreshPriceListContext({ preserveSelection: true }).then(() => this.refreshAllItemPricing()).catch((error) => { this.saveError = errorMessage(error, "Unable to refresh Buying Price List."); }); } },
		async setBranch(next) { const branch = next || ""; this.values.branch = branch; this.values.warehouse = ""; this.pricingCache.clear(); if (!branch || !this.values.company) return; const token = ++this.cascadeToken; try { const r = await resolveBranchWarehouse({ company: this.values.company, branch, preference: "purchase" }); if (token !== this.cascadeToken) return; this.values.branch = r.branch || branch; this.values.warehouse = r.warehouse || ""; await this.refreshPriceListContext(); this.refreshAllItemPricing(); } catch (error) { if (token === this.cascadeToken) this.saveError = errorMessage(error, "Unable to resolve the Branch receiving stock location."); } },
