			...
			<div v-else class="transaction-page-content">
				...
				<form v-if="!savedDocument || editingSavedDraft" class="edge-panel transaction-form" @submit.prevent="saveDraft">
					...
					<div class="field-grid">
						...
						<EdgeLinkField :modelValue="values.price_list" label="Buying Price List" placeholder="Select assigned Price List" description="Branch default takes precedence. Otherwise choose from Price Lists assigned to you for this Branch." :required="Boolean(formContext.pricing?.selection_required)" :disabled="editingSavedDraft || Boolean(formContext.pricing?.locked) || !Boolean(formContext.pricing?.can_select)" :searcher="searchPriceList" :context="searchContext" @update:modelValue="setPriceList" />
...
export default {
	...
	methods: {
		...
		setSupplier(next) { const value = next || ""; const changed = this.values.supplier !== value; this.values.supplier = value; this.pricingCache.clear(); if (changed) { this.values.items = this.values.items.map((row) => ({ ...row, rate: "" })); this.refreshPriceListContext({ preserveSelection: true }).then(() => this.refreshAllItemPricing()).catch((error) => { this.saveError = errorMessage(error, "Unable to refresh Buying Price List."); }); } },
