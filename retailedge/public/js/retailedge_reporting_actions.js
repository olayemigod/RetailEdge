(() => {
	...
	function activeReportKey() {
		...
		return SHELL_REPORT_ROUTES[path] || "";
	}

	function reportParentFilters(component) {
		const parent = component?.$parent;
		if (typeof parent?.providerFilters === "function") {
			return parent.providerFilters() || {};
		}
	...
	function reportParentPageState(component) {
		const parent = component?.$parent;
		const pageLength = Math.max(1, Number(parent?.filters?.page_size || 50));
		const page = Math.max(1, Number(parent?.currentPage || parent?.pagination?.page || 1));
		return {
			pageLength,
			start: Math.max(0, (page - 1) * pageLength),
		};
	}
	...
	function reportActionError(error, fallback) {
		...
		if (frappe?.msgprint) {
			frappe.msgprint({ title: __("Reporting Action Failed"), message, indicator: "red" });
		}
		return message;
	}
	...
	function installShellGovernance(runtime = window.EdgeSuiteUI) {
		...
		const GovernedReportShell = defineComponent({
			name: "RetailEdgeGovernedReportShell",
			inheritAttrs: false,
			data() {
				return {
			...
			methods: {
				async refreshCapabilities() {
					...
					if (!reportKey) return;
					try {
						this.capabilities = await getCapabilities(reportKey);
					} catch (error) {
						this.capabilities = { can_view: true, can_print: false, can_export: false };
						console.warn("RetailEdge report capabilities unavailable", error);
					}
				},
				async handleExport(options) {
					const reportKey = activeReportKey();
					if (!reportKey || !this.capabilities.can_export) return;
					const { start, pageLength } = reportParentPageState(this);
					this.exportBusy = true;
					try {
						await exportReport({
							reportKey,
							filters: reportParentFilters(this),
							options,
							start,
							pageLength,
						});
					} catch (error) {
						reportActionError(error, "The report could not be exported.");
					} finally {
				...
				async handlePrint() {
					const reportKey = activeReportKey();
					if (!reportKey || !this.capabilities.can_print) return;
					this.printBusy = true;
					try {
						await printReport({ reportKey, filters: reportParentFilters(this) });
					} catch (error) {
						reportActionError(error, "The report print view could not be prepared.");
					} finally {
			...
			render() {
				const reportKey = activeReportKey();
				if (!reportKey) return h(baseReportShell, this.$attrs, this.$slots);
				const rows = Array.isArray(this.$attrs.rows) ? this.$attrs.rows : [];
				return h(baseReportShell, {
					...this.$attrs,
					exportEnabled: Boolean(rows.length && this.capabilities.can_export),
					printEnabled: Boolean(rows.length && this.capabilities.can_print),
		...
		const GovernedLegacyExportMenu = defineComponent({
			name: "RetailEdgeGovernedLegacyExportMenu",
			inheritAttrs: false,
			render() {
				if (activeReportKey()) return null;
				return h(baseExportMenu, this.$attrs, this.$slots);
			},
		});

		runtime.registerComponent("EdgeReportShell", GovernedReportShell, { replace: true });
		runtime.registerComponent("EdgeExportMenu", GovernedLegacyExportMenu, { replace: true });
		shellGovernanceInstalled = true;
		return true;
	}
	...
	window.addEventListener("edgesuite:report-runtime-ready", () => installShellGovernance(window.EdgeSuiteUI));
})();
