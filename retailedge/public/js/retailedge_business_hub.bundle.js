...
function mountRetailEdgeBusinessHub(target) {
	if (typeof window === 'undefined' || !window.EdgeUI) {
		throw new Error('EdgeSuite UI runtime not loaded.');
	}
	if (typeof window.EdgeUI.createEdgeApp !== 'function') {
		throw new Error('EdgeSuite UI createEdgeApp API is unavailable.');
	}
	return window.EdgeUI.createEdgeApp(RetailEdgeBusinessHub, target);
}
