<template>
	<EdgeModal
		...
		subtitle="Preview ERPNext's sourcing mapping before any RFQ draft or supplier email is created."
		size="xl"
		@close="close"
	>
		<div class="rfq-preview">
			...
			<div class="rfq-preview__actions">
				<button type="button" class="edge-button edge-button--primary" :disabled="loading || !suppliers.length" @click="loadPreview">
					{{ loading ? 'Preparing preview…' : 'Preview RFQ' }}
				</button>
			</div>

			<EdgeLoadingState v-if="loading" message="Preparing RFQ preview..." :skeleton="true" />
			<EdgeErrorState v-else-if="error" title="RFQ preview unavailable" :message="error" @retry="loadPreview" />
			<div v-else-if="preview" class="rfq-preview__result">
				...
				<div class="rfq-preview__ready">
					...
					<span>No RFQ draft has been saved and no supplier email has been sent.</span>
				</div>
		...
		<template #footer>
			<div class="rfq-preview__footer">
				<button v-if="nativeFallbackEnabled && preview" type="button" class="edge-button" @click="openAdvanced">Advanced: Prepare Draft in ERPNext</button>
...
export default {
	...
	methods: {
		...
		async loadPreview() {
			if (!this.materialRequest || !this.suppliers.length || this.loading) return;
			this.loading = true;
			this.error = "";
			this.preview = null;
			try {
		...
		openAdvanced() {
			if (!this.nativeFallbackEnabled || !this.preview) return;
...
.rfq-preview__source span,.rfq-preview__context span,.rfq-preview__table small { display:block; opacity:.72; }
...
.rfq-preview__context div,.rfq-preview__ready { padding:.75rem; border:1px solid var(--border-color,#d1d8dd); border-radius:.5rem; }
