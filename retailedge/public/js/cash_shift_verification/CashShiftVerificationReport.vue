		...
		>
			<template #filters>
				<div class="shift-filter-grid">
					...
					<label class="edge-field"><span class="edge-field-label">Cash Status</span><select v-model="filters.cash_status" class="edge-input"><option value="">All</option><option v-for="status in cashStatuses" :key="status" :value="status">{{ status }}</option></select></label>
				...
				<details class="advanced-filters">
					...
					<div class="shift-filter-grid advanced-grid">
						<label class="edge-field"><span class="edge-field-label">Review Status</span><select v-model="filters.review_status" class="edge-input"><option value="">All</option><option v-for="status in reviewStatuses" :key="status" :value="status">{{ status }}</option></select></label>
						<label class="edge-check"><input v-model="filters.only_unsynced" type="checkbox" true-value="1" false-value="0" /><span>Only shifts with unsynced cash invoices</span></label>
					</div>
				</details>
			</template>
			<template #resultMeta>
				<span>{{ scan.rows || 0 }} matching shift{{ Number(scan.rows || 0) === 1 ? "" : "s" }}</span>
				<span>Bounded verification dataset · {{ providerDatasetLimit.toLocaleString() }} row cap</span>
...
export default {
	...
	data() {
		return {
			...
			filters: { company: "", branch: "", pos_profile: "", cashier: "", cash_status: "", review_status: "", only_unsynced: 0, from_date: "", to_date: "", page_size: 50 },
			cashStatuses: ["Balanced", "Shortage", "Overage", "Needs Review", "Missing Closing Shift", "Missing Opening Shift"],
			reviewStatuses: ["Draft", "Ready for Review", "In Review", "Balanced", "Variance Found", "Clarification Required", "Approved", "Rejected", "Cancelled", "Reopened"],
		};
	},
	computed: {
		...
		reportColumns() { return (this.columns || []).map((column) => ({ ...column, fieldtype: column.fieldtype || "Data", clickable: ["daily_sales_audit", "cashier", "pos_profile", "opening_shift", "closing_shift"].includes(column.fieldname) })); },
	},
	...
	methods: {
		async fetchMetadata() {
			...
			try {
				...
				const [context, navigation] = await Promise.all([callMethod("retailedge.cash_shift_verification.get_cash_shift_verification_context"), navigationPromise]);
			...
			} catch (error) { this.error = errorMessage(error, "Failed to load Cash Shift Verification controls."); }
		...
		async searchOptions(kind, txt) { const result = await callMethod("retailedge.cash_shift_verification.search_cash_shift_verification_options", { kind, txt, company: this.filters.company }); return Array.isArray(result) ? result : []; },
		...
		async fetchData() {
			if (!this.filters.company) return; if (!this.reportProvider?.load) { this.error = "The shared EdgeSuite Cash Shift Verification provider is unavailable."; return; }
			...
			} catch (error) { this.rows = []; this.columns = []; this.summary = []; this.error = errorMessage(error, "Cash Shift Verification failed to load."); }
			finally { this.loading = false; }
		},
		goToPage(page) { const next = Math.max(1, Number(page || 1)); if (next === this.currentPage) return; this.currentPage = next; this.fetchData(); }, setPageSize(pageSize) { this.filters.page_size = Number(pageSize || 50); this.currentPage = 1; this.fetchData(); }, rowKey(row, index) { return row.daily_sales_audit || `${row.shift_date || "shift"}:${row.cashier || ""}:${index}`; },
		handleCellClick(payload) { const column = payload?.column; const row = payload?.row; if (!column || !row) return; const value = row[column.fieldname]; if (!value) return; if (column.fieldname === "daily_sales_audit") frappe.set_route("Form", "RetailEdge Daily Sales Audit", value); else if (column.fieldname === "cashier") frappe.set_route("Form", "User", value); else if (column.fieldname === "pos_profile") frappe.set_route("Form", "POS Profile", value); else if (column.fieldname === "opening_shift") frappe.set_route("Form", "POS Opening Shift", value); else if (column.fieldname === "closing_shift") frappe.set_route("Form", "POS Closing Shift", value); },
		formatCell(value, column) { if (value === null || value === undefined || value === "") return "—"; if (column.fieldtype === "Currency") { try { return frappe.format(Number(value), { fieldtype: "Currency" }); } catch (_error) { return Number(value).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }); } } if (column.fieldtype === "Int") return Number(value).toLocaleString(); if (column.fieldtype === "Date") { try { return frappe.datetime.str_to_user(`${value} 00:00:00`).split(" ")[0]; } catch (_error) { return String(value); } } return String(value); },
	},
};
</script>

<style scoped>
.shift-fallback { margin:20px; padding:24px; border:1px solid var(--edge-border,#d9d9d9); border-radius:var(--edge-radius-lg,10px); background:var(--edge-surface,#fff); display:flex; flex-direction:column; gap:8px; }
.shift-filter-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:var(--edge-space-md,16px); align-items:end; width:100%; }
...
.edge-check { min-height:38px; display:flex; align-items:center; gap:8px; color:var(--edge-text,#101828); }
...
@media (max-width:1180px) { .shift-filter-grid { grid-template-columns:repeat(3,minmax(0,1fr)); } }
@media (max-width:860px) { .shift-filter-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } }
@media (max-width:560px) { .shift-filter-grid { grid-template-columns:1fr; } }
</style>