<template>
	...
	<EdgeAppShell
		...
		:userName="userName"
		:menuItems="menuItems"
		activeRoute="/app/money-overview"
		:hideNativeSidebar="true"
		@navigate="handleNavigation"
	>
		<EdgeDashboardShell
			title="Money Overview"
			eyebrow="Liquidity & Working Capital"
			subtitle="Selected-period cash movement alongside current receivables and payables. Period net change is a flow metric, not a closing cash or bank balance."
			...
			loadingMessage="Building money overview…"
			@retry="fetchData"
			@export="handleExport"
			@print="handlePrint"
		>
			...
			<EdgeDashboardGrid minColumnWidth="20rem">
				<EdgeDashboardSection v-if="attention.length" title="Attention Required" description="Working-capital exceptions from the existing RetailEdge receivable and payable reports." span="2">
					<div class="money-attention-list">
						<button v-for="item in attention" :key="`${item.section}-${item.metric}`" type="button" class="money-attention-item" @click="openRoute(item.route)">
							...
							<strong>{{ formatCard(item) }}</strong>
						</button>
					</div>
				</EdgeDashboardSection>

				<EdgeDashboardSection v-for="section in availableSections" :key="section.key" :title="section.label" :description="section.time_basis_label || sectionDescription(section)">
					<div class="money-section-cards">
						<div v-for="card in section.summary" :key="card.label" class="money-metric">
							<span>{{ card.label }}</span><strong>{{ formatCard(card) }}</strong>
						</div>
					</div>
					<button type="button" class="edge-button edge-button--secondary money-open" @click="openSection(section)">Open {{ section.label }}</button>
				</EdgeDashboardSection>
				...
				<EdgeDashboardSection v-if="unavailableSections.length" title="Restricted Sections" description="These views remain hidden because your current permissions do not allow their source reports.">
					<ul class="money-restricted"><li v-for="section in unavailableSections" :key="section.key"><strong>{{ section.label }}</strong> — {{ section.reason }}</li></ul>
				</EdgeDashboardSection>
			</EdgeDashboardGrid>
		</EdgeDashboardShell>
	</EdgeAppShell>
		...
		async fetchData() {
			...
			try {
				const [result, capabilities] = await Promise.all([
					callMethod("retailedge.money_dashboard.get_money_dashboard_data", { filters: this.filters }),
					getDashboardCapabilities(DASHBOARD_KEY, this.filters),
				]);
				this.sections = result.sections || {}; this.headlineSummary = result.headline_summary || []; this.attention = result.attention || []; this.capabilities = capabilities || this.capabilities;
			} catch (error) { this.sections = {}; this.headlineSummary = []; this.attention = []; this.error = errorMessage(error, "Money Overview failed to load."); }
...
.money-attention-list { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
.money-attention-item { display: flex; justify-content: space-between; gap: 14px; padding: 12px 14px; border: 1px solid var(--edge-border); border-radius: 8px; background: var(--edge-surface); color: var(--edge-text); text-align: left; cursor: pointer; }
.money-attention-item span { display: grid; gap: 3px; }
.money-open { margin-top: 14px; }
.money-restricted { margin: 0; padding-left: 18px; display: grid; gap: 8px; }
@media (max-width: 720px) { .money-overview-filters, .money-section-cards, .money-attention-list { grid-template-columns: 1fr; } }
</style>
