...
export default {
	...
	methods: {
		...
		restoreRecovery() { if (!this.recoveryCandidate?.values) return; this.values = { ...this.values, ...clone(this.recoveryCandidate.values), items: (this.recoveryCandidate.values.items || []).map((row) => ({ ...row })) }; this.recoveryCandidate = null; this.handoffNotice = "Recovered unsaved transfer work from this browser session."; },
