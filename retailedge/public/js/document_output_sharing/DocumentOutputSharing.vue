<template>
	...
	>
		<EdgePageLayout class="retailedge-document-output-page">
			<EdgePageHeader
				title="Document Output & Sharing"
			...
			<div v-else class="output-content">
				...
				<div class="output-grid">
					<section class="edge-panel output-selector">
						...
						<EdgeEmptyState
							v-else-if="searchAttempted && selectedDocumentKey"
							title="No permitted documents found"
							description="Try another document number or check the current Operating Company and Branch."
						/>
					</section>
					...
					<section class="edge-panel output-actions">
						<div class="section-heading">
							...
							<p v-if="details">{{ details.party || "Customer document" }}<span v-if="details.grand_total"> · {{ details.currency }} {{ formatAmount(details.grand_total) }}</span></p>
						</div>
						...
						<div v-else class="action-form">
							<div class="field-grid">
								...
								<label class="letterhead-toggle">
									<input v-model="useLetterhead" type="checkbox" />
									<span>Use ERPNext Letterhead</span>
								</label>
							</div>
							...
							<div class="share-section">
								...
								<div class="field-grid">
									<div>
										<label class="field-label" for="output-email">Recipient</label>
										<input id="output-email" v-model="emailRecipient" class="edge-control" type="email" :disabled="!details.can_email" placeholder="customer@example.com" />
									</div>
									<div>
										<label class="field-label" for="output-subject">Subject</label>
										<input id="output-subject" v-model="emailSubject" class="edge-control" type="text" :disabled="!details.can_email" />
									</div>
								</div>
							...
							<div class="share-section">
								<h4>WhatsApp</h4>
								<p>RetailEdge does not publish a private PDF to create a WhatsApp link. Download the PDF, then open the prepared message and attach the PDF in WhatsApp.</p>
...
export default {
	...
	computed: {
		...
		canSendEmail() {
			return Boolean(this.details?.can_email && this.details?.can_print && String(this.emailRecipient || "").trim());
		},
	},
	created() {
	...
	methods: {
		...
		async selectDocument(row) {
			...
			try {
				...
				this.printFormat = this.details.print_formats?.[0] || "Standard";
				this.emailRecipient = this.details.contact_email || "";
				this.emailSubject = `${this.selectedDefinition?.label || this.details.doctype} ${this.details.name}`;
				this.emailMessage = `Please find attached ${this.selectedDefinition?.label || this.details.doctype} ${this.details.name}.`;
			} catch (error) {
				this.details = null;
		...
		printQuery() {
			return new URLSearchParams({
				doctype: this.details.doctype,
				name: this.details.name,
				format: this.printFormat || "Standard",
				no_letterhead: this.useLetterhead ? "0" : "1",
			}).toString();
		},
		previewDocument() {
			if (!this.details?.can_print) return;
			window.open(`/printview?${this.printQuery()}`, "_blank", "noopener,noreferrer");
		},
		downloadPdf() {
			if (!this.details?.can_print) return;
		...
		openNativeDocument() {
			if (!this.details?.native_route) return;
			window.open(this.details.native_route, "_blank", "noopener,noreferrer");
		},
		async sendEmail() {
			if (!this.canSendEmail || this.sendingEmail) return;
		...
		async openWhatsApp() {
			...
			try {
				...
				const target = phone ? `https://wa.me/${phone}?text=${encodeURIComponent(handoff.text || "")}` : `https://wa.me/?text=${encodeURIComponent(handoff.text || "")}`;
...
<style scoped>
.retailedge-document-output-page { padding-bottom: 2rem; }
.output-content { display: grid; gap: 1rem; }
.output-policy, .section-heading { display: flex; justify-content: space-between; gap: 1rem; align-items: flex-start; }
.output-policy h3, .section-heading h3, .share-section h4 { margin: 0.2rem 0 0.35rem; }
.output-policy p, .section-heading p, .share-section p { margin: 0; color: var(--text-muted); }
.output-kicker { display: block; font-size: 0.75rem; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; color: var(--text-muted); }
.output-grid { display: grid; grid-template-columns: minmax(18rem, 0.8fr) minmax(24rem, 1.2fr); gap: 1rem; }
.output-selector, .output-actions { display: grid; gap: 0.9rem; align-content: start; }
.field-label { display: block; font-size: 0.82rem; font-weight: 600; margin-bottom: 0.3rem; }
.edge-control { width: 100%; min-height: 2.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius-md, 0.5rem); background: var(--control-bg, var(--card-bg)); color: var(--text-color); padding: 0.55rem 0.7rem; }
.edge-textarea { resize: vertical; }
.search-row, .primary-actions { display: flex; gap: 0.6rem; flex-wrap: wrap; }
.search-row .edge-control { flex: 1 1 12rem; }
.search-results { display: grid; gap: 0.4rem; max-height: 19rem; overflow: auto; }
.search-result { display: grid; gap: 0.2rem; text-align: left; padding: 0.65rem 0.75rem; border: 1px solid var(--border-color); border-radius: var(--border-radius-md, 0.5rem); background: var(--card-bg); color: var(--text-color); }
.search-result:hover { background: var(--subtle-fg, var(--control-bg)); }
.search-result span { font-size: 0.8rem; color: var(--text-muted); }
.action-form { display: grid; gap: 1rem; }
.field-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.75rem; align-items: end; }
.letterhead-toggle { display: flex; align-items: center; gap: 0.5rem; min-height: 2.5rem; }
.share-section { display: grid; gap: 0.65rem; padding-top: 1rem; border-top: 1px solid var(--border-color); }
@media (max-width: 900px) {
	.output-grid, .field-grid { grid-template-columns: 1fr; }
	.output-policy { flex-direction: column; }
}
</style>
