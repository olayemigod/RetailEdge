...
export default {
	...
	methods: {
		...
		resetDocumentSelection() { this.searchText = ""; this.searchResults = []; this.searchAttempted = false; this.details = null; this.printFormat = "Standard"; },
		...
		async selectDocument(row) { const name = String(row?.value || row?.name || "").trim(); if (!name || !this.selectedDocumentKey) return; this.searchText = name; this.searchResults = []; this.detailsLoading = true; try { this.details = await callMethod(DETAILS_METHOD, { document: this.selectedDocumentKey, name }); this.printFormat = this.details.print_formats?.[0] || "Standard"; this.emailRecipient = this.details.contact_email || ""; this.emailSubject = `${this.selectedDefinition?.label || this.details.doctype} ${this.details.name}`; this.emailMessage = `Please find attached ${this.selectedDefinition?.label || this.details.doctype} ${this.details.name}.`; } catch (error) { this.details = null; frappe.msgprint({ title: __("Document unavailable"), message: errorMessage(error, "Unable to open this document."), indicator: "red" }); } finally { this.detailsLoading = false; } },
