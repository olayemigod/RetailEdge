...
def get_edgepay_bank_match_confirmation_preflight(evidence_name, review_name=None):
	...
	if review.payment_entry != evidence.payment_entry and review.suggested_document != evidence.payment_entry:
		return {
			"ok": False,
			"message": f"Bank Match Review {review_name} is not linked to Payment Entry {evidence.payment_entry}.",
		}
	...
	if review.decision_status == "Confirmed":
		return {
			"ok": True,
			"message": "Bank Match Review is already confirmed.",
	...
	if review.decision_status in ("Rejected", "Cancelled"):
		return {
			"ok": False,
			"message": f"Bank Match Review {review_name} is {review.decision_status} and cannot be confirmed.",
		}
	...
	if not frappe.db.exists("Bank Transaction", review.bank_transaction):
		return {
			"ok": False,
			"message": f"Bank Transaction {review.bank_transaction} linked on review does not exist.",
		}
	...
	if diff > max(tolerance, 0.01):
		return {
			"ok": False,
			"message": f"Amount mismatch: Bank Transaction deposit {bt.deposit} does not match evidence amount {evidence.amount} within tolerance {tolerance}.",
		}

	if bt.currency and evidence.currency and bt.currency.upper() != evidence.currency.upper():
		return {
			"ok": False,
			"message": f"Currency mismatch: Bank Transaction currency {bt.currency} does not match evidence currency {evidence.currency}.",
		}
	...
	if duplicate_pe_match:
		return {
			"ok": False,
			"message": f"Payment Entry {evidence.payment_entry} already has another confirmed bank match review: {duplicate_pe_match}.",
		}
	...
	if duplicate_bt_match:
		return {
			"ok": False,
			"message": f"Bank Transaction {review.bank_transaction} already has another confirmed bank match review: {duplicate_bt_match}.",
		}
...
def mark_edgepay_evidence_reconciliation_matched(evidence_name, review_name):
	...
	evidence.db_set(
		...
		f"Bank Match Review confirmed: {review_name} for Bank Transaction {review.bank_transaction}.",
	)
