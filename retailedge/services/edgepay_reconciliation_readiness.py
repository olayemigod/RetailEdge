...
def get_edgepay_reconciliation_readiness(evidence_name):
	...
	if evidence.submission_status != "Submitted":
		return {
			"ok": False,
			"status": "Not Ready",
			"message": f"Payment Evidence submission status is not Submitted. Current status: {evidence.submission_status}.",
		}
	...
	if evidence.posting_status != "Submitted":
		return {
			"ok": False,
			"status": "Not Ready",
			"message": f"Payment Evidence posting status is not Submitted. Current status: {evidence.posting_status}.",
		}
	...
	if pe_doc.docstatus != 1:
		...
		if pe_doc.docstatus == 2:
			return {
				"ok": False,
				"status": "Blocked",
				"message": f"Linked Payment Entry {evidence.payment_entry} is cancelled.",
			}
		return {
			"ok": False,
			"status": "Blocked",
	...
	if not evidence.provider_reference:
		return {
			"ok": False,
			"status": "Blocked",
			"message": "Missing provider_reference on payment evidence.",
		}
	...
	if abs(flt(evidence.amount) - flt(pe_doc.paid_amount)) > 0.01:
		return {
			"ok": False,
			"status": "Blocked",
			"message": f"Amount mismatch: evidence amount {evidence.amount} does not match Payment Entry paid amount {pe_doc.paid_amount}.",
		}

	# Currency checking: allow if it matches either paid_from or paid_to account currency
	currency_matches = False
	if (
		pe_doc.paid_from_account_currency
		and evidence.currency.upper() == pe_doc.paid_from_account_currency.upper()
	):
		currency_matches = True
	if (
		pe_doc.paid_to_account_currency
		and evidence.currency.upper() == pe_doc.paid_to_account_currency.upper()
	):
		currency_matches = True
	...
	if not currency_matches:
		return {
			"ok": False,
			"status": "Blocked",
			"message": f"Currency mismatch: evidence currency {evidence.currency} does not match Payment Entry account currencies.",
		}
	...
	reconciliations = frappe.get_all(
		...
		filters={"payment_document": "Payment Entry", "payment_entry": evidence.payment_entry},
		fields=["parent"],
	)
	...
	confirmed_match = frappe.get_all(
		"RetailEdge Bank Transaction Match",
		filters={"payment_entry": evidence.payment_entry, "decision_status": "Confirmed"},
...
def find_edgepay_payment_entry_bank_match_candidates(evidence_name):
	...
	for bt in bts:
		...
		# Date gap check
		target_date = (
			getdate(evidence.paid_on)
			if evidence.paid_on
			else (getdate(pe_doc.reference_date) if pe_doc else None)
		)
