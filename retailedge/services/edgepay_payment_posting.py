...
def get_edgepay_evidence_posting_preflight(evidence_name):
	...
	if evidence.request_status != "Paid":
		return {
			"ok": False,
			"message": f"Payment Request status is not Paid. Current status: {evidence.request_status}.",
		}
	...
	if evidence.transaction_status and evidence.transaction_status.upper() not in ("SUCCESS", "PAID"):
		return {
			"ok": False,
			"message": f"Transaction status is not successful. Current status: {evidence.transaction_status}.",
		}
	...
	if not frappe.db.exists(evidence.source_doctype, evidence.source_name):
		return {
			"ok": False,
			"message": f"Source document {evidence.source_doctype} {evidence.source_name} does not exist.",
		}
	...
	if abs(flt(evidence.amount) - flt(source_amount)) > 0.01:
		return {
			"ok": False,
			"message": f"Amount mismatch: evidence amount {evidence.amount} does not match source amount {source_amount}.",
		}
	...
	if not source_currency or evidence.currency.upper() != source_currency.upper():
		return {
			"ok": False,
			"message": f"Currency mismatch: evidence currency {evidence.currency} does not match source currency {source_currency}.",
		}
	...
	if evidence.payment_entry:
		return {
			"ok": False,
			"message": f"Payment Entry {evidence.payment_entry} is already linked to this evidence.",
		}

	# 10. no conflicting existing Payment Entry / payment allocation already covers the same source and provider reference
	conflicts = frappe.get_all(
		"Payment Entry",
		filters={"reference_no": evidence.provider_reference, "docstatus": ["<", 2]},
	...
	if conflicts:
		return {
			"ok": False,
			"message": f"Conflicting Payment Entry {conflicts[0].name} already exists with reference_no {evidence.provider_reference}.",
		}
...
def prepare_edgepay_payment_entry_draft(evidence_name):
	...
	if evidence.payment_entry:
		if frappe.db.exists("Payment Entry", evidence.payment_entry):
			...
			if pe_docstatus == 0:
				...
				evidence.db_set(
					"posting_preflight_message",
					_("Payment Entry {0} is already submitted for this transaction.").format(
						evidence.payment_entry
					),
				)
				frappe.throw(
					_("Payment Entry {0} is already submitted for this transaction.").format(
	...
	if evidence.provider_reference:
		existing_draft = frappe.db.get_value(
			"Payment Entry", {"reference_no": evidence.provider_reference, "docstatus": 0}, "name"
		)
		if existing_draft:
		...
		existing_submitted = frappe.db.get_value(
			"Payment Entry", {"reference_no": evidence.provider_reference, "docstatus": 1}, "name"
		)
		if existing_submitted:
			...
			frappe.throw(
				_("Payment Entry {0} is already submitted for this transaction.").format(existing_submitted)
			)
	...
	return {"ok": True, "payment_entry": pe.name, "message": _("Draft Payment Entry created successfully.")}
...
def get_edgepay_payment_entry_submission_preflight(evidence_name):
	...
	if evidence.posting_status not in ("Draft Created", "Ready"):
		return {
			"ok": False,
			"message": f"Posting status is not Draft Created or Ready. Current status: {evidence.posting_status}.",
		}
	...
	if pe_docstatus != 0:
		return {
			"ok": False,
			"message": f"Linked Payment Entry {evidence.payment_entry} is already in status {pe_docstatus}.",
		}
	...
	if abs(pe_amount - flt(evidence.amount)) > 0.01:
		return {
			"ok": False,
			"message": f"Payment Entry amount {pe_amount} does not match evidence amount {evidence.amount}.",
		}
	...
	if not has_matching_ref:
		return {
			"ok": False,
			"message": f"Payment Entry does not allocate to source document {evidence.source_doctype} {evidence.source_name}.",
		}
	...
	conflicts = frappe.get_all(
		"Payment Entry",
		filters={"reference_no": evidence.provider_reference, "docstatus": 1},
		fields=["name"],
	)
	...
	if conflicts:
		return {
			"ok": False,
			"message": f"Conflicting submitted Payment Entry {conflicts[0]} already exists with reference_no {evidence.provider_reference}.",
		}
	...
	if outstanding_amount < (flt(evidence.amount) - 0.01):
		return {
			"ok": False,
			"message": f"Outstanding amount {outstanding_amount} is less than evidence amount {evidence.amount}.",
		}
...
def submit_edgepay_payment_entry(evidence_name):
	...
	if evidence.provider_reference:
		...
		if conflicting and conflicting != evidence.payment_entry:
			...
			evidence.db_set(
				"submission_message", _("Conflicting submitted Payment Entry {0} found.").format(conflicting)
			)
			frappe.throw(
				_("Conflicting submitted Payment Entry {0} found for provider reference {1}.").format(
					conflicting, evidence.provider_reference
				)
			)
	...
	try:
		...
		return {"ok": True, "payment_entry": pe.name, "message": _("Payment Entry submitted successfully.")}
