...
from erpnext.accounts.doctype.payment_request.payment_request import get_amount, get_gateway_details
...
REUSABLE_PAYMENT_REQUEST_STATUSES = {"Requested", "Initiated", "Partially Paid", "Failed"}
...
def _assert_payable_invoice(invoice, customers: list[str]) -> None:
	...
	if invoice.docstatus != 1 or invoice.is_return:
		frappe.throw(_("Only submitted sales invoices with an amount due can be paid online."), frappe.ValidationError)
...
def _existing_payment_request(invoice) -> Any | None:
	rows = frappe.get_all(
		"Payment Request",
		filters={"reference_doctype": "Sales Invoice", "reference_name": invoice.name, "docstatus": 1, "status": ["in", sorted(REUSABLE_PAYMENT_REQUEST_STATUSES)]},
...
def _payment_result(invoice, payment_request, *, reused: bool) -> dict[str, Any]:
	...
	if not payment_url:
		frappe.throw(_("Online payment is not available for this invoice. Please contact the business for payment assistance."), frappe.ValidationError)
	return {"invoice": invoice.name, "payment_request": payment_request.name, "status": payment_request.status, "payment_url": payment_url, "amount": flt(payment_request.outstanding_amount or payment_request.grand_total), "currency": payment_request.currency or invoice.currency, "reused": reused}
...
def _create_payment_request(invoice):
	...
	if not gateway.get("name") or not gateway.get("payment_gateway") or not gateway.get("payment_account"):
		frappe.throw(_("Online payment is not configured for this company. Please contact the business for payment assistance."), frappe.ValidationError)
	if str(gateway.get("payment_channel") or "").strip() == "Phone":
		frappe.throw(_("Online redirect payment is not configured for this company. Please contact the business for payment assistance."), frappe.ValidationError)
	...
	payment_request.update({"payment_gateway_account": gateway.get("name"), "payment_gateway": gateway.get("payment_gateway"), "payment_account": gateway.get("payment_account"), "payment_channel": gateway.get("payment_channel"), "payment_request_type": "Inward", "currency": invoice.currency, "party_account_currency": party_account_currency, "grand_total": amount, "mode_of_payment": None, "email_to": frappe.session.user, "subject": _("Payment Request for {0}").format(invoice.name), "message": gateway.get("message") or _("Please use the secure payment link to pay this invoice."), "reference_doctype": "Sales Invoice", "reference_name": invoice.name, "company": invoice.company, "party_type": "Customer", "party": invoice.customer, "party_name": invoice.customer_name, "bank_account": get_party_bank_account("Customer", invoice.customer), "make_sales_invoice": 0, "mute_email": 1, "cost_center": invoice.get("cost_center"), "project": invoice.get("project")})
...
def request_invoice_payment(invoice_name: str) -> dict[str, Any]:
	...
	return _payment_result(invoice, _create_payment_request(invoice), reused=False)
