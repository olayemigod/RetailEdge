...
REPORT_SORT_FIELDS: dict[str, frozenset[str]] = {
	...
	"expense-analysis": frozenset({"group_label", "expense_lines", "posted_lines", "unposted_cashier_lines", "gross_spend", "credits_reversals", "net_expense", "posted_net_expense", "unposted_cashier_exposure", "average_expense", "posting_blocked_count"}),
	...
	"payment-settlement-analysis": frozenset({"group_label", "payment_count", "receive_count", "pay_count", "transfer_count", "money_in", "money_out", "net_external_settlement", "transfer_amount", "customer_receipts_allocated", "customer_advance_available", "average_external_payment", "multi_currency_exception_count"}),
	"purchase-analysis": frozenset({"group_label", "purchased_qty", "returned_qty", "net_qty", "purchase_value", "returns_value", "net_purchased", "invoice_count", "average_transaction_value", "average_unit_cost"}),
	...
	"supplier-performance": frozenset({"supplier", "supplier_name", "purchase_value", "returns_value", "net_purchased", "invoice_count", "average_invoice_value", "return_rate_percent", "current_outstanding", "overdue_outstanding", "open_bill_count", "overdue_bill_count", "oldest_overdue_days"}),
	...
	"sales-analysis": frozenset({"group_label", "sold_qty", "returned_qty", "net_qty", "sales_value", "returns_value", "net_sales", "invoice_count", "average_transaction_value", "average_selling_price", "recorded_cost", "gross_profit", "gross_margin_percent", "missing_recorded_cost"}),
