...
class TestInventoryActionSummary(unittest.TestCase):
	@patch(
		"retailedge.inventory_health.get_hidden_inventory_location_exceptions",
		return_value=_empty_locations(),
	)
	@patch(
	...
	def test_demand_failure_does_not_remove_current_stock_or_reorder_actions(
		self, stock, replenishment, _demand, _locations
	):
		...
		self.assertEqual(cards["Negative Stock Locations Hidden by Aggregate"], 0)
		self.assertNotIn("Non-moving", cards)
		self.assertTrue(result["metadata"]["current_stock_available"])
		self.assertTrue(result["metadata"]["location_exceptions"]["available"])
	...
	def test_reorder_failure_does_not_remove_current_stock_or_movement_actions(
		self, stock, _replenishment, demand, _locations
	):
		stock.return_value = _stock_payload(
			{
				"item_code": "ITEM-2",
	...
	@patch("retailedge.inventory_health.get_inventory_replenishment", return_value={"items": []})
	@patch("retailedge.inventory_health.get_hidden_inventory_location_exceptions")
	@patch("retailedge.inventory_health._build_stock_position_dataset")
	def test_hidden_location_cards_are_additive_to_legacy_item_cards(
		self, stock, locations, _replenishment, _demand
	):
		stock.return_value = _stock_payload(
			{
				"item_code": "ITEM-3",
		...
		locations.return_value = {
			"summary": {
				"hidden_negative_location_count": 1,
				"hidden_fully_reserved_location_count": 2,
			}
		}
		...
		self.assertEqual(cards["Available Items"], 1)
		self.assertEqual(cards["Negative Stock"], 0)
		self.assertEqual(cards["Fully Reserved"], 0)
		self.assertEqual(cards["Negative Stock Locations Hidden by Aggregate"], 1)
		self.assertEqual(cards["Fully Reserved Locations Hidden by Aggregate"], 2)
		self.assertTrue(result["metadata"]["location_exceptions"]["available"])
