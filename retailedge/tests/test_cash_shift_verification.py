...
class CashShiftVerificationTests(unittest.TestCase):
	...
	def test_cash_shift_verification_status_handles_missing_shifts(self):
		self.assertEqual(
			get_cash_status({"opening_shift": None, "closing_shift": "CLOSE-1", "cash_variance": 0}),
			"Missing Opening Shift",
		)
		self.assertEqual(
			get_cash_status({"opening_shift": "OPEN-1", "closing_shift": None, "cash_variance": 0}),
			"Missing Closing Shift",
		)

	@patch(
		"retailedge.retailedge.report.retailedge_cash_shift_verification.retailedge_cash_shift_verification.get_cash_invoice_sync_counts",
		return_value=(2, 1),
	)
	@patch(
		"retailedge.retailedge.report.retailedge_cash_shift_verification.retailedge_cash_shift_verification.has_doctype",
		return_value=True,
	)
	@patch(
		"retailedge.retailedge.report.retailedge_cash_shift_verification.retailedge_cash_shift_verification.resolve_cash_shift_verification_read_scope"
	)
	@patch(
		"retailedge.retailedge.report.retailedge_cash_shift_verification.retailedge_cash_shift_verification.frappe.get_all"
	)
	def test_cash_shift_verification_executes_and_uses_expected_cash_formula(
		self, mock_get_all, mock_read_scope, _mock_has_doctype, _mock_sync_counts
	):
		mock_read_scope.return_value = {"company": "Process Edge (Demo)"}
