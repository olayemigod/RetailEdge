...
class BankStatementImportSetupTests(unittest.TestCase):
	@patch("retailedge.bank_statement_import.resolve_retailedge_bank_account")
	@patch("retailedge.bank_statement_import.frappe")
	def test_creates_native_import_with_validated_company_branch_and_bank_account(
		self, mock_frappe, mock_resolve
	):
		mock_resolve.return_value = {"bank_account": "Access Bank Ketu - Access Bank"}
		account = MagicMock()
		...
		result = create_bank_statement_import(
	...
	@patch("retailedge.bank_statement_import.resolve_retailedge_bank_account")
	@patch("retailedge.bank_statement_import.frappe")
	def test_rejects_bank_account_outside_operational_scope(self, mock_frappe, mock_resolve):
		mock_resolve.side_effect = RuntimeError("branch/company mismatch")
	...
	@patch("retailedge.bank_statement_import.resolve_retailedge_bank_account")
	@patch("retailedge.bank_statement_import.frappe")
	def test_requires_company_and_bank_account(self, mock_frappe, mock_resolve):
	...
	@patch("retailedge.bank_statement_import.resolve_retailedge_bank_account")
	@patch("retailedge.bank_statement_import.frappe")
	def test_template_search_is_permission_aware_bank_and_branch_scoped(
		self, mock_frappe, mock_resolve
	):
		mock_resolve.return_value = {"bank_account": "Access Bank Ketu - Access Bank"}
		account = MagicMock()
	...
	@patch("retailedge.bank_statement_import.resolve_retailedge_bank_account")
	@patch("retailedge.bank_statement_import.frappe")
	def test_template_create_is_bounded_to_native_debit_credit_mapping(
		self, mock_frappe, mock_resolve
	):
		mock_resolve.return_value = {"bank_account": "Access Bank Ketu - Access Bank"}
		account = MagicMock()
	...
	@patch("retailedge.bank_statement_import.resolve_retailedge_bank_account")
	@patch("retailedge.bank_statement_import.frappe")
	def test_apply_template_preserves_branch_scope_and_native_erpnext_import(
		self, mock_frappe, mock_resolve
	):
		...
		result = apply_bank_statement_template(import_doc.name, template.name, "Lagos")
