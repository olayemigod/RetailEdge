...
def test_setup_context_is_permission_aware():
    source = _source(SETUP_PY)
    assert 'frappe.has_permission(doctype, ptype="read")' in source
    ...
    assert 'frappe.db.count(doctype)' in source
...
def test_expense_categories_are_managed_inside_setup_with_safe_cascades():
    ...
    assert 'companyField.df.onchange = () =>' in js_source
...
def test_branch_profiles_are_managed_inside_setup_and_reuse_controller_validation():
    py_source = _source(SETUP_PY)
    js_source = _source(SETUP_JS)
    controller_source = _source(APP_ROOT / "branch_profile.py")
    ...
    assert "doc.insert()" in py_source and "doc.save()" in py_source
...
def test_branch_operational_user_backend_is_safe_and_role_bound():
    ...
    assert "def get_branch_profile_details(name: str)" in source
...
def test_retailedge_settings_are_managed_through_singleton_with_permissions():
    py_source = _source(SETUP_PY)
    js_source = _source(SETUP_JS)
    assert '"key": "settings"' in py_source
    assert '"managed_in_page": True' in py_source
    ...
    assert 'resource.key === "settings"' in js_source
    assert '__("RetailEdge Settings")' in js_source
...
def test_settings_operating_role_tables_are_safe_and_managed_in_ui():
    ...
    assert "doc.set(table_field, _normalize_roles" in py_source
    assert "function roleTableField(fieldname, label, rows)" in js_source
    assert 'options: "Role"' in js_source


def test_settings_ui_manages_posting_and_cost_controls_but_excludes_bank_and_platform_controls():
    py_source = _source(SETUP_PY)
    js_source = _source(SETUP_JS)
    ...
    for fieldname in (
        ...
        assert fieldname in managed_block
        assert f'fieldname: "{fieldname}"' in js_source
    excluded = (
...
def test_statement_mappings_are_managed_inside_setup_with_backend_validation():
    py_source = _source(SETUP_PY)
    js_source = _source(SETUP_JS)
    assert '"key": "statement_mappings"' in py_source
    ...
    assert 'resource.key === "statement_mappings"' in js_source
    ...
    assert '__("Add Mapping Template")' in js_source
    assert '__("Edit Mapping Template")' in js_source


def test_statement_mapping_company_change_filters_and_clears_default_account():
    source = _source(SETUP_JS)
    assert 'options: "Account"' in source
