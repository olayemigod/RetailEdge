...
class TestProjectExpenseRouting(unittest.TestCase):
	...
	def test_routes_cover_native_project_procurement_material_and_cost_lifecycle(self, mock_has_field, mock_can_create, mock_get_doc, _mock_assert_read):
		...
		self.assertIn("no generic project expense or procurement ledger", result["policy"])
		self.assertNotIn("RetailEdge", result["policy"])

	@patch("retailedge.project_expense_routing._assert_read")
	@patch("retailedge.project_expense_routing.frappe.get_doc")
	@patch("retailedge.project_expense_routing._can_create")
	@patch("retailedge.project_expense_routing._has_field")
	def test_unavailable_or_unpermitted_native_routes_are_not_exposed(self, mock_has_field, mock_can_create, mock_get_doc, _mock_assert_read):
	...
	@patch("retailedge.project_expense_routing._can_create")
	@patch("retailedge.project_expense_routing._has_field")
	def test_missing_parent_project_field_does_not_fake_project_prefill(self, mock_has_field, mock_can_create, mock_get_doc, _mock_assert_read):
		mock_get_doc.return_value = SimpleNamespace(name="PROJ-0001", company="Demo Company", cost_center="")
	...
	@patch("retailedge.project_expense_routing._can_create")
	@patch("retailedge.project_expense_routing._has_field")
	def test_expense_claim_is_optional_and_does_not_fake_parent_project_field(self, mock_has_field, mock_can_create, mock_get_doc, _mock_assert_read):
		mock_get_doc.return_value = SimpleNamespace(name="PROJ-0001", company="Demo Company", cost_center="")
