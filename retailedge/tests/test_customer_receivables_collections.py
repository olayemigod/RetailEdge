...
class TestCustomerReceivablesCollections(TestCase):
	...
	def test_edgesuite_report_exposes_post_only_native_draft_handoffs(self):
		...
		self.assertIn("Payment Entry opens in EdgeSuite and remains user-controlled", self.component)
		self.assertIn("Advanced collection handoffs remain available when permitted", self.component)
