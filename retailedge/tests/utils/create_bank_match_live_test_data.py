...
def _ensure_account(company, account_name, account_type, root_type, parent_root):
	...
	if existing:
		return existing
	parent = _first_value(
		"Account", {"company": company, "root_type": root_type, "is_group": 1, "account_name": parent_root}
	)
	if not parent:
...
def _context():
	...
	) or _ensure_account(company, f"{PREFIX} Sales", "Income Account", "Income", "Income")
	receivable = _first_value(
		"Account", {"company": company, "account_type": "Receivable", "is_group": 0}
	) or _ensure_account(company, f"{PREFIX} Receivable", "Receivable", "Asset", "Accounts Receivable")
	cost_center = _first_value("Cost Center", {"company": company, "is_group": 0})
	bank_account = _first_value(
		"Account", {"company": company, "account_type": "Bank", "is_group": 0}
	) or _ensure_account(company, f"{PREFIX} Bank Ledger", "Bank", "Asset", "Bank Accounts")
	alt_bank_account = _ensure_account(company, f"{PREFIX} Alt Bank Ledger", "Bank", "Asset", "Bank Accounts")
	cash_account = _first_value(
		"Account", {"company": company, "account_type": "Cash", "is_group": 0}
	) or _ensure_account(company, f"{PREFIX} Cash Ledger", "Cash", "Asset", "Cash In Hand")
...
def _bank_transaction(
	ctx, amount, idx, scenario, reference, account=None, posting_date=None, description_extra=""
):
...
def _create_exact(
	...
	pe = _payment_entry(
		...
		idx,
		scenario,
		account=pe_account or ctx.bank_account,
		mop=ctx.bank_mop,
		reference=ref,
	)
	bt = _bank_transaction(
		ctx,
		bt_amount if bt_amount is not None else amount,
		idx,
		scenario,
		ref,
		account=bt_account or ctx.bank_transaction_account,
		posting_date=si.posting_date,
		description_extra=pe.name,
	)
	return si, pe, bt
...
def _create_pos_row(ctx, idx, amount=None):
	...
	si = _sales_invoice(
		ctx, idx, amount, "pos_payment_row", is_pos=True, mop=mop, account=ctx.bank_account, reference=ref
	)
	bt = _bank_transaction(
		ctx,
		amount,
		...
		"pos_payment_row",
		ref,
		account=ctx.bank_transaction_account,
		posting_date=si.posting_date,
		description_extra=si.name,
	)
	return si, None, bt
...
def _create_all(counts):
	...
	for _ in range(counts["exact_payment_entry"]):
		...
		created["payment_entries"].append(pe.name)
		created["bank_transactions"].append(bt.name)
		summary["exact_payment_entry"] += 1
		idx += 1
		if idx % 25 == 0:
			frappe.db.commit()
	...
	for _ in range(counts["cash_excluded"]):
		...
		created["sales_invoices"].append(si.name)
		summary["cash_excluded"] += 1
		idx += 1
		if idx % 25 == 0:
			frappe.db.commit()
	for n in range(counts["duplicate_pressure"]):
		si, pe, bt = _create_exact(
			ctx, idx, scenario="duplicate_pressure", amount=2222, reference=f"{PREFIX}-DUP-{n // 2:04d}"
		)
		created["sales_invoices"].append(si.name)
		created["payment_entries"].append(pe.name)
		created["bank_transactions"].append(bt.name)
		summary["duplicate_pressure"] += 1
		idx += 1
		if idx % 25 == 0:
			frappe.db.commit()
	for n in range(counts["amount_mismatch"]):
		...
		si, pe, bt = _create_exact(
			...
			amount=amount,
			bt_amount=amount + (1 if n % 2 else -1),
			reference=f"{PREFIX}-MM-{n:04d}",
		)
		created["sales_invoices"].append(si.name)
		created["payment_entries"].append(pe.name)
		created["bank_transactions"].append(bt.name)
	...
	for n in range(counts["bank_account_mismatch"]):
		si, pe, bt = _create_exact(
			ctx,
			idx,
			scenario="bank_account_mismatch",
			pe_account=ctx.bank_account,
			bt_account=ctx.alt_bank_transaction_account,
			reference=f"{PREFIX}-BAM-{n:04d}",
		)
		created["sales_invoices"].append(si.name)
		created["payment_entries"].append(pe.name)
		created["bank_transactions"].append(bt.name)
		summary["bank_account_mismatch"] += 1
		idx += 1
		if idx % 25 == 0:
			frappe.db.commit()
	for n in range(counts["broken_candidate"]):
		bt = _bank_transaction(
			...
			idx,
			"broken_candidate",
			f"{PREFIX}-BROKEN-{n:04d}",
			account=ctx.bank_transaction_account,
		)
		review = _review(
			ctx,
			idx,
			"broken_candidate_missing_candidate",
			bt=bt,
			candidate_doctype=None,
			candidate_name=None,
			amount=999 + n,
		...
		created["bank_transactions"].append(bt.name)
		created["reviews"].append(review.name)
		summary["broken_candidate"] += 1
		idx += 1
		if idx % 25 == 0:
			frappe.db.commit()
	for n in range(counts["already_handled"]):
		...
		review = _review(
			ctx,
			idx,
			"already_handled_observability",
			bt=bt,
			candidate_doctype="Payment Entry",
			candidate_name=pe.name,
			amount=flt(pe.paid_amount),
			status="Confirmed",
			details={"source": PREFIX, "scenario": "already_handled"},
		)
		review.execution_status = "Already Handled"
		review.execution_message = (
			f"{PREFIX} observability sample: already handled state for manual UI testing."
		)
...
def smoke_checks(prefix=PREFIX, create_batch_job=0):
	...
	rows = get_bank_transaction_matching_rows(
		filters={"include_confirmed_matches": 1, "show_already_in_review": 1}
	)
	...
	payload = {
		...
		"cash_only_records": frappe.db.count(
			"Sales Invoice", [["remarks", "like", f"%{prefix}%cash_excluded%"]]
		),
		"mismatch_records": frappe.db.count(
			"Bank Transaction", [["description", "like", f"%{prefix}%amount_mismatch%"]]
		),
		"duplicate_pressure_records": frappe.db.count(
			"Bank Transaction", [["description", "like", f"%{prefix}%duplicate_pressure%"]]
		),
		"bank_account_mismatch_records": frappe.db.count(
			"Bank Transaction", [["description", "like", f"%{prefix}%bank_account_mismatch%"]]
		),
		"matching_report_total_rows": len(rows),
		"matching_report_tagged_rows": len(tagged_rows),
		"expected_eligible": frappe.db.count(
			"Bank Transaction",
			[["description", "like", f"%{prefix}%"], ["description", "not like", "%broken_candidate%"]],
		),
		"expected_excluded": frappe.db.count(
			"Sales Invoice", [["remarks", "like", f"%{prefix}%cash_excluded%"]]
		),
		"expected_require_review": (
			...
			+ frappe.db.count(
				...
				[["match_reason", "like", f"%{prefix}%broken_candidate%"]],
			)
		),
...
def create_confirmed_execution_sample():
	...
	for row in rows:
		...
		review = _review(
			...
			details={
				"source": PREFIX,
				"scenario": "confirmed_execution_ready",
				"payment_event_source": "Payment Entry",
			},
		)
		frappe.db.commit()
		payload = {
			"match": review.name,
			"bank_transaction": row["bank_transaction"],
			"payment_entry": row["suggested_document"],
			"prefix": PREFIX,
		}
		print(json.dumps(payload, indent=2, default=str))
		return payload
	frappe.throw(
		"No unused tagged Payment Entry/Bank Transaction pair was available for confirmed execution sample."
	)
...
def light_smoke_checks():
	...
	report_rows = get_bank_transaction_matching_rows(
		filters={"include_confirmed_matches": 1, "show_already_in_review": 1}, limit=5
	)
	suggestion_rows = _tagged_payment_entry_suggestion_rows(limit=201)
	background_required = retailedge_api.create_bank_match_reviews_from_suggestions(
		rows=json.dumps(suggestion_rows, default=str), run_background=0
	)
	batch_job = retailedge_api.create_bank_match_reviews_from_suggestions(
		rows=json.dumps(suggestion_rows, default=str), run_background=1
	)
	payload = {
		"sales_invoices": frappe.db.count("Sales Invoice", _tag_filters("Sales Invoice", PREFIX)),
		"payment_entries": frappe.db.count("Payment Entry", _tag_filters("Payment Entry", PREFIX)),
		...
		"batch_job": batch_job,
		"batch_job_rows": frappe.db.count(
			"RetailEdge Bank Match Batch Job Row", {"parent": batch_job.get("batch_job")}
		)
		if batch_job and batch_job.get("batch_job")
		else 0,
	}
	frappe.db.commit()
	print(json.dumps(payload, indent=2, default=str))
...
def bounded_smoke_checks():
	...
	rows = get_bank_transaction_matching_rows(
		filters={"include_confirmed_matches": 1, "show_already_in_review": 1}, limit=260
	)
	tagged_rows = [row for row in rows if PREFIX in json.dumps(row, default=str)]
	batch_result = (
		create_bank_match_reviews_from_suggestions(rows=tagged_rows[:201])
		if len(tagged_rows) >= 201
		else None
	)
	payload = {
		"sales_invoices": frappe.db.count("Sales Invoice", _tag_filters("Sales Invoice", PREFIX)),
