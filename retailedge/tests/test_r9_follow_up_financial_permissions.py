...
class RetailEdgeR9FollowUpFinancialPermissionTests(unittest.TestCase):
	...
	def test_global_operational_user_cannot_list_r9_financial_followups(self, _action_role, _owner_access, _global):
	...
	@patch("retailedge.action_follow_up._has_action_center_role", return_value=True)
	def test_direct_r9_followup_form_read_requires_owner_financial_access(self, _action_role, _company, _owner_access):
	...
	@patch("retailedge.action_follow_up._has_owner_financial_access", return_value=False)
	@patch("retailedge.action_follow_up._has_action_center_role", return_value=True)
	def test_r9_assignee_query_excludes_user_without_owner_financial_access(self, _action_role, _owner_access):
		self.assertFalse(
			_is_assignable_user(
				"ops@example.com",
				company="Example Co",
