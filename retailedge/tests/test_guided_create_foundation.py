...
class TestGuidedCreateFoundation(unittest.TestCase):
	...
	def test_business_hub_exposes_one_permission_filtered_create_picker(self):
		...
		self.assertIn("runQuickAction(action)", component)
	...
	def test_create_picker_routes_all_registered_actions_to_guided_adapters(self):
		...
		self.assertIn("CashierExpenseDetailDialog", component)
		self.assertIn("this.cashierExpenseWorkflowName = name", component)
		self.assertIn("this.cashierExpenseWorkflowOpen = true", component)
		self.assertNotIn("openNativeCashierExpense", component)
