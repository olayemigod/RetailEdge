...
def test_numeric_sort_keeps_missing_values_last_in_both_directions():
	rows = [
	...
	assert [row["item_code"] for row in ascending] == ["A", "B", "MISSING"]
	assert [row["item_code"] for row in descending] == ["B", "A", "MISSING"]


@patch("retailedge.inventory_health._build_inventory_health_dataset")
def test_public_service_sorts_complete_dataset_before_pagination(build_dataset):
	...
	assert [row["item_code"] for row in result["rows"]] == ["HIGH", "MID", "LOW"]
	assert result["metadata"]["sort"] == {
		"field": "available_qty",
		"direction": "desc",
		"fieldtype": "Float",
	}
...
@patch("retailedge.inventory_health._build_inventory_health_dataset")
def test_public_service_rejects_unknown_sort_field(build_dataset):
	build_dataset.return_value = {
