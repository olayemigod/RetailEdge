...
def test_bank_exception_summary_classifies_existing_match_state_without_candidate_discovery(monkeypatch):
	...
	rows = [
		frappe._dict(name="M1", transaction_date="2026-08-01", bank_amount=100, decision_status="Suggested", execution_status="Not Executed"),
		frappe._dict(name="M2", transaction_date="2026-08-02", bank_amount=200, decision_status="Confirmed", execution_status="Not Executed"),
		frappe._dict(name="M3", transaction_date="2026-08-03", bank_amount=300, decision_status="Confirmed", execution_status="Blocked"),
		frappe._dict(name="M4", transaction_date="2026-08-04", bank_amount=400, decision_status="Confirmed", execution_status="Failed"),
	]
	with patch("retailedge.bank_exception_summary.frappe.get_list", return_value=rows):
		result = summary.get_bank_exception_summary({"company": "Test Company", "from_date": "2026-08-01", "to_date": "2026-08-20"})
...
def test_bank_exception_summary_uses_bounded_permission_aware_get_list(monkeypatch):
	frappe.session.user = "Administrator"
	monkeypatch.setattr(frappe, "has_permission", lambda *args, **kwargs: True)
	with patch("retailedge.bank_exception_summary.frappe.get_list", return_value=[]) as get_list:
		summary.get_bank_exception_summary({"company": "Test Company", "branch": "HQ", "from_date": "2026-08-01", "to_date": "2026-08-20"})
