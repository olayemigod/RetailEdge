...
class BankingPageContractTests(unittest.TestCase):
	...
	def test_page_enhancements_compact_filters_and_import_inside_edgesuite_modal(self):
		...
		self.assertIn("Import Template", enhancements)
		self.assertIn("Create Template", enhancements)
		self.assertIn("get_bank_statement_template_options", enhancements)
		self.assertIn("create_bank_statement_template", enhancements)
		self.assertIn("apply_bank_statement_template", enhancements)
		self.assertIn("Save & Select Template", enhancements)
		self.assertIn("search_bank_matching_bank_accounts", enhancements)
		self.assertIn('label: t("Branch")', enhancements)
		self.assertIn("searcher: searchBankAccounts", enhancements)
		self.assertIn('branch: fieldValueByLabel(filterBar, "Branch")', enhancements)
		self.assertIn('branch: state.branch || ""', enhancements)
		...
		self.assertIn("retailedge-bank-import-template-row", css)
		self.assertIn("retailedge-bank-template-grid", css)
		self.assertIn("retailedge-bank-layout", css)
		self.assertIn("font-size: .92rem", css)
		self.assertIn("min-height: 1.95rem", DENSE_CSS.read_text())
