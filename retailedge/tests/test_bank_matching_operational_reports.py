...
class BankMatchingOperationalReportsTests(unittest.TestCase):
	...
	def test_r53_report_js_forces_live_refresh_behavior(self):
		report_paths = [
			str(
				APP_ROOT
				/ "retailedge/report/retailedge_unmatched_bank_transactions/retailedge_unmatched_bank_transactions.js"
			),
			str(
				APP_ROOT
				/ "retailedge/report/retailedge_unmatched_bank_payment_events/retailedge_unmatched_bank_payment_events.js"
			),
			str(
				APP_ROOT
				/ "retailedge/report/retailedge_bank_match_reconciliation_readiness/retailedge_bank_match_reconciliation_readiness.js"
			),
		]
		for report_path in report_paths:
			with self.subTest(report_path=report_path):
	...
	def test_unmatched_bank_transactions_exclude_active_and_confirmed_by_default(
		self, mock_rows, mock_normalize, mock_matches, mock_build
	):
		...
		mock_build.side_effect = lambda bank_transaction, matches, filters: {
	...
	def test_unmatched_bank_payment_events_exclude_cash_rows_and_keep_non_cash_amount(
		self, _mock_doctype, _mock_payment_entry_events, mock_invoice_rows, _mock_review_state
	):
		mock_invoice_rows.return_value = [
			{
				...
				"customer": "Palmer",
				"customer_name": "Palmer Productions",
				"retailedge_branch": "HQ",
				"payment_row_index": 1,
				"mode_of_payment": "Cash",
				"payment_account": "Cash - PED",
				"payment_amount": 500,
	...
	@patch(
		"retailedge.bank_matching_operational_reports._get_candidate_review_state",
		return_value=({}, {}, {"ACC-PAY-2026-00008"}),
	)
	@patch(
		"retailedge.bank_matching_operational_reports._get_payment_entry_sales_invoice_references",
		return_value={},
	)
	@patch("retailedge.bank_matching_operational_reports._get_payment_entry_event_source_rows")
	@patch("retailedge.bank_matching_operational_reports.has_doctype", return_value=True)
	def test_confirmed_payment_entry_is_excluded_from_unmatched_events_by_default(
		self, _mock_doctype, mock_rows, _mock_refs, _mock_review_state, _mock_sales_invoice_events
	):
		mock_rows.return_value = [
			{
				"name": "ACC-PAY-2026-00008",
	...
	def test_reconciliation_readiness_marks_confirmed_exact_payment_entry_match_ready(
		self, mock_get_all, _mock_doctype, mock_account_payload, mock_hydrate
	):
	...
	@patch("retailedge.bank_matching_operational_reports.has_doctype", return_value=True)
	@patch("retailedge.bank_matching_operational_reports.frappe.get_all")
	def test_reconciliation_readiness_flags_account_mismatch_as_exception(
		self, mock_get_all, _mock_doctype, mock_account_payload, mock_hydrate
	):
		mock_get_all.return_value = [
			{
	...
	@patch(
		"retailedge.retailedge.report.retailedge_unmatched_bank_transactions.retailedge_unmatched_bank_transactions.get_unmatched_bank_transaction_rows"
	)
	def test_unmatched_bank_transactions_report_executes_from_current_helper_data(self, mock_rows):
		...
		_columns, rows, _message, _chart, _summary = execute_unmatched_bank_transactions_report(
			{"from_date": "2026-05-01", "to_date": "2026-05-31"}
		)
		self.assertEqual(rows, [{"bank_transaction": "ACC-BTN-2026-00008"}])

	@patch(
		"retailedge.retailedge.report.retailedge_unmatched_bank_payment_events.retailedge_unmatched_bank_payment_events.get_unmatched_bank_payment_event_rows"
	)
	def test_unmatched_bank_payment_events_report_executes_from_current_helper_data(self, mock_rows):
		...
		_columns, rows, _message, _chart, _summary = execute_unmatched_payment_events_report(
			{"from_date": "2026-05-01", "to_date": "2026-05-31"}
		)
		self.assertEqual(rows, [{"payment_event_document": "ACC-PAY-2026-00008"}])

	@patch(
		"retailedge.retailedge.report.retailedge_bank_match_reconciliation_readiness.retailedge_bank_match_reconciliation_readiness.get_bank_match_reconciliation_readiness_rows"
	)
	def test_reconciliation_readiness_report_executes_from_current_helper_data(self, mock_rows):
		...
		_columns, rows, _message, _chart, _summary = execute_readiness_report(
			{"from_date": "2026-05-01", "to_date": "2026-05-31"}
		)
...
class BankMatchingOperationalReportFilterTests(unittest.TestCase):
	@patch(
		"retailedge.bank_matching_operational_reports._find_candidate_bank_transaction_for_event",
		return_value=None,
	)
	@patch(
		"retailedge.bank_matching_operational_reports._active_review_match_for_candidate", return_value=None
	)
	@patch(
		"retailedge.bank_matching_operational_reports.payment_entry_has_active_confirmed_bank_match",
		return_value=False,
	)
	@patch(
		"retailedge.bank_matching_operational_reports._get_payment_entry_sales_invoice_references",
	...
	@patch("retailedge.bank_matching_operational_reports._sales_invoice_payment_event_rows", return_value=[])
	@patch("retailedge.bank_matching_operational_reports.has_doctype", return_value=True)
	def test_payment_entry_event_rows_exclude_cash_and_filter_payment_account(
		self,
		_mock_doctype,
		_mock_sales_rows,
		mock_source_rows,
		...
		rows = get_unmatched_bank_payment_event_rows(
			{"from_date": "2026-05-01", "to_date": "2026-05-31", "payment_account": "Demo Bank Account - PED"}
		)
		self.assertEqual(len(rows), 1)
	...
	def test_invoice_payment_event_rows_filter_mode_of_payment_and_resolved_account(
		...
		mock_invoice_rows.return_value = [
			...
			},
			{
				...
				"payment_account": "Other Account - PED",
				"payment_amount": 500,
...
class BankMatchingOperationalReportPerformanceGuardsTests(unittest.TestCase):
	@patch("retailedge.bank_matching_operational_reports._build_unmatched_bank_transaction_row")
	@patch(
		"retailedge.bank_matching_operational_reports._get_existing_matches_by_bank_transaction",
		return_value={},
	)
	@patch("retailedge.bank_matching_operational_reports.normalize_bank_transaction")
	@patch("retailedge.bank_matching_operational_reports._get_bank_transaction_rows")
	def test_unmatched_bank_transactions_enforce_row_limit(
		self, mock_rows, mock_normalize, _mock_matches, mock_build
	):
		mock_rows.return_value = [{"name": f"BT-{idx:04d}"} for idx in range(550)]
		mock_normalize.side_effect = lambda row: {
		...
		rows = get_unmatched_bank_transaction_rows(
			{"from_date": "2026-05-01", "to_date": "2026-05-31"}, limit=500
		)
