...
class TestEdgePayPaymentPosting(FrappeTestCase):
	def setUp(self):
		...
		self.get_pe_patcher = patch(
			"retailedge.services.edgepay_payment_posting.get_payment_entry",
			side_effect=self.mock_get_payment_entry,
		)
	...
	def test_manual_status_marking(self):
		...
		self.assertEqual(
			frappe.db.get_value("RetailEdge EdgePay Payment Evidence", "EPE-TEST-010", "posting_status"),
			"Ready",
		)

		res_blocked = mark_edgepay_evidence_posting_blocked(
			"EPE-TEST-010", reason="Blocked for manual verification"
		)
		...
		self.assertEqual(
			...
			"Blocked for manual verification",
		)
	...
	def test_submission_missing_source_document_blocks(self):
		...
		frappe.db.set_value(
			"RetailEdge EdgePay Payment Evidence", "EPE-SUB-006", "source_name", "SINV-RE-MISSING"
		)
	...
	def test_duplicate_submitted_provider_reference_blocks(self):
		...
		self.assertEqual(
			frappe.db.get_value("RetailEdge EdgePay Payment Evidence", "EPE-SUB-008", "posting_status"),
			"Blocked",
		)
	...
	def test_failed_submission_stores_redacted_error(self):
		...
		try:
			...
			self.assertEqual(
				ev.submission_message,
				"Error occurred during Payment Entry submission. Details redacted for security.",
			)
		finally:
			self._mock_submit_failure = False
	...
	def test_submission_blocked_marking(self):
		...
		self.assertTrue(res["ok"])
		self.assertEqual(
			frappe.db.get_value("RetailEdge EdgePay Payment Evidence", "EPE-SUB-011", "posting_status"),
			"Blocked",
		)
		self.assertEqual(
			frappe.db.get_value("RetailEdge EdgePay Payment Evidence", "EPE-SUB-011", "submission_status"),
			"Blocked",
		)
		self.assertEqual(
			frappe.db.get_value("RetailEdge EdgePay Payment Evidence", "EPE-SUB-011", "submission_message"),
			"Manual hold",
		)
