...
def execute(filters=None):
	...
	for ev in evidences:
		...
		blocking_reason = (
			ev.reconciliation_message if ev.reconciliation_status in ("Blocked", "Exception") else ""
		)
...
def get_columns():
	return [
		...
		{
			"label": _("Blocking Reason"),
			"fieldname": "blocking_reason",
...
def get_report_summary(rows):
	return [
		{
			"label": _("Reconciled"),
			"value": sum(1 for row in rows if row.get("reconciliation_status") == "Reconciled"),
			"datatype": "Int",
			"indicator": "Green",
		},
		{
			"label": _("Ready"),
			"value": sum(1 for row in rows if row.get("reconciliation_status") == "Ready"),
			"datatype": "Int",
			"indicator": "Blue",
		},
		{
			"label": _("Matched"),
			"value": sum(1 for row in rows if row.get("reconciliation_status") == "Matched"),
			"datatype": "Int",
			"indicator": "Orange",
		},
		{
			"label": _("Blocked / Not Ready"),
			"value": sum(
				1 for row in rows if row.get("reconciliation_status") in ("Blocked", "Exception", "Not Ready")
			),
			"datatype": "Int",
			"indicator": "Red",
		},
