					...
					frappe.call({
						...
						callback: function (r) {
							...
							if (candidates.length === 0) {
								frappe.msgprint(
									__(
										"No candidate Bank Transactions found matching this evidence."
									)
								);
								return;
							}
							...
							let options = candidates.map((c) => ({
								...
								label: `${c.bank_transaction} - Date: ${c.date} | Ref: ${
							...
							frappe.prompt(
									...
									frappe.call({
										...
										callback: function (res) {
											if (res.message && res.message.ok) {
												...
												frappe.call({
													...
													args: {
														...
														bank_transaction_name:
															cand_values.bank_transaction,
													},
													callback: function (create_res) {
														if (
															create_res.message &&
															create_res.message.ok
														) {
															frappe.show_alert({
																message: __(
																	"Bank Match Review {0} created successfully.",
																	[
																		create_res.message
																			.review_name,
																	]
															...
															frappe.msgprint(
																create_res.message.message ||
																	__(
																		"Failed to create match review."
																	)
															);
														}
													},
												...
												frappe.msgprint(
													res.message.message ||
														__("Preflight validation failed.")
												);
											}
										},
									});
								...
								__("Select Candidate"),
								__("Create Match Review")
							);
						},
					});
				},
				__("Select Payment Evidence"),
				__("Next")
			);
		});
		...
		report.page.add_inner_button(__("Confirm Match Review"), () => {
			frappe.prompt(
				...
				(values) => {
					let evidence_name = values.evidence;
					frappe.db
						.get_doc("RetailEdge EdgePay Payment Evidence", evidence_name)
						.then((doc) => {
							let review_name = doc.linked_bank_match_review;
							if (!review_name) {
								frappe.msgprint(
									__("No linked Bank Match Review found on evidence.")
								);
								return;
							}
							frappe.call({
								...
								callback: function (res) {
									if (res.message && res.message.ok) {
										frappe.confirm(
											__(
												"Are you sure you want to confirm this Bank Match Review?"
											),
											() => {
												frappe.call({
													...
													callback: function (confirm_res) {
														if (
															confirm_res.message &&
															confirm_res.message.ok
														) {
															frappe.show_alert({
																message: __(
																	"Bank Match Review confirmed successfully."
																),
																indicator: "green",
															});
															report.refresh();
															...
															frappe.msgprint(
																confirm_res.message.message ||
																	__(
																		"Failed to confirm match review."
																	)
															);
														}
													},
										...
										frappe.msgprint(
											res.message.message ||
												__("Confirmation preflight validation failed.")
										);
									}
								},
							});
				...
				__("Select Payment Evidence to Confirm"),
				__("Confirm")
			);
		});
	},
