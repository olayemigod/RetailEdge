...
def get_columns(filters):
	...
	return [
		...
		{"label": _("Compare UOM"), "fieldname": "compare_uom", "fieldtype": "Link", "options": "UOM", "width": 105, "hidden": 0 if compare_uom else 1},
		{"label": _("Compare In Quantity"), "fieldname": "compare_in_quantity", "fieldtype": "Float", "width": 135, "hidden": 0 if compare_uom else 1},
		{"label": _("Compare Out Quantity"), "fieldname": "compare_out_quantity", "fieldtype": "Float", "width": 140, "hidden": 0 if compare_uom else 1},
		{"label": _("Compare Balance"), "fieldname": "compare_balance", "fieldtype": "Float", "width": 125, "hidden": 0 if compare_uom else 1},
		...
		{"label": _("Voucher Number"), "fieldname": "voucher_no", "fieldtype": "Dynamic Link", "options": "voucher_type", "width": 155},
...
def resolve_warehouse_scope(filters):
	...
	if branches and not branch_warehouses:
		frappe.throw(_("No warehouse scope could be resolved for the selected or permitted branch."), frappe.PermissionError)
	...
	if branch_warehouses and warehouse not in branch_warehouses:
		frappe.throw(_("Warehouse {0} is outside the selected or permitted Branch scope.").format(warehouse), frappe.PermissionError)
	return [warehouse]
...
def get_branch_warehouses(company: str, branch: str) -> set[str]:
	...
	if branch_field:
		warehouses.update(frappe.get_all("Warehouse", filters={"company": company, branch_field: branch}, pluck="name", limit_page_length=0))
	if frappe.db.exists("DocType", "RetailEdge Branch Profile"):
		fields = ["name"] + [field for field in BRANCH_PROFILE_WAREHOUSE_FIELDS if has_field("RetailEdge Branch Profile", field)]
		for profile in frappe.get_all("RetailEdge Branch Profile", filters={"company": company, "branch": branch, "enabled": 1}, fields=fields, limit_page_length=0):
...
def expand_group_warehouses(company: str, warehouse_names: set[str]) -> set[str]:
	if not warehouse_names:
		return set()
	rows = frappe.get_all("Warehouse", filters={"company": company, "name": ["in", sorted(warehouse_names)]}, fields=["name", "is_group", "lft", "rgt"], limit_page_length=0)
	resolved = {row.name for row in rows if not row.is_group}
	for row in rows:
		if row.is_group:
			resolved.update(frappe.get_all("Warehouse", filters={"company": company, "is_group": 0, "lft": [">", row.lft], "rgt": ["<", row.rgt]}, pluck="name", limit_page_length=0))
	return resolved
...
def get_stock_ledger_rows(filters, warehouse_scope=None):
	...
	rows = frappe.get_list(
		...
		fields=["name", "posting_datetime", "posting_date", "posting_time", "item_code", "warehouse", "actual_qty", "qty_after_transaction", "voucher_type", "voucher_no", "voucher_detail_no", "batch_no", "serial_no", "stock_uom", "creation"],
...
def split_opening_stock_reconciliations(filters, stock_ledger_rows, opening_balance, *, reconciliation_purposes=None):
	"""ERPNext behaviour: From Date Opening Stock reconciliations seed the opening row."""
	candidate_rows = [row for row in stock_ledger_rows if row.voucher_type == "Stock Reconciliation" and getdate(row.posting_date) == getdate(filters.from_date)]
	if not candidate_rows:
		return flt(opening_balance), list(stock_ledger_rows), None
	purposes = reconciliation_purposes if reconciliation_purposes is not None else get_stock_reconciliation_purposes(candidate_rows)
	...
	for row in sorted(stock_ledger_rows, key=stock_ledger_sort_key):
		if row.voucher_type == "Stock Reconciliation" and getdate(row.posting_date) == getdate(filters.from_date) and purposes.get(row.voucher_no) == "Opening Stock":
			resolved_balance = flt(row.qty_after_transaction)
			opening_names.add(row.name)
			opening_context = frappe._dict({"voucher_no": row.voucher_no, "posting_datetime": row.posting_datetime, "qty_after_transaction": resolved_balance})
	return resolved_balance, [row for row in stock_ledger_rows if row.name not in opening_names], opening_context
...
def get_stock_reconciliation_purposes(rows):
	...
	if not names:
		return {}
	return {row.name: row.purpose for row in frappe.get_all("Stock Reconciliation", filters={"name": ["in", names]}, fields=["name", "purpose"], limit_page_length=0)}


def stock_ledger_sort_key(row):
	return (get_datetime(row.get("posting_datetime")) if row.get("posting_datetime") else datetime.min, get_datetime(row.get("creation")) if row.get("creation") else datetime.min, row.get("name") or "")


def output_row_sort_key(row):
	return (get_datetime(row.get("posting_datetime")) if row.get("posting_datetime") else datetime.min, get_datetime(row.get("creation")) if row.get("creation") else datetime.min, row.get("sort_key") or "")
...
def build_movement_rows(stock_ledger_rows, filters, warehouse_scope=None, *, item=None, conversion_map=None, opening_balance=None):
	...
	data = [
		build_stock_entry_movement_row(rows, detail_map, header_map.get("Stock Entry", {}), item, conversion_map, filters.get("compare_uom"))
		for rows in grouped_stock_entries.values()
	]
	data.extend(build_single_movement_row(sle, header_map.get(sle.voucher_type, {}).get(sle.voucher_no, {}), item, conversion_map, filters.get("compare_uom")) for sle in other_rows)
	data = sorted([row for row in data if row], key=output_row_sort_key)
	return apply_running_balances(data, get_opening_balance(filters, warehouse_scope) if opening_balance is None else opening_balance)


def get_conversion_map(item_codes, compare_uom):
	if not compare_uom or not item_codes:
		return {}
	rows = frappe.get_all("UOM Conversion Detail", filters={"parent": ["in", item_codes], "parenttype": "Item", "uom": compare_uom}, fields=["parent", "uom", "conversion_factor"], limit_page_length=0)
...
def get_stock_entry_detail_map(stock_ledger_rows):
	names = sorted({row.voucher_detail_no for row in stock_ledger_rows if row.voucher_type == "Stock Entry" and row.voucher_detail_no})
	if not names:
		return {}
	fields = ["name", "parent", "item_code", "item_name", "s_warehouse", "t_warehouse", "qty", "transfer_qty", "stock_uom", "uom", "conversion_factor"]
	for optional in ("batch_no", "serial_and_batch_bundle"):
		if has_field("Stock Entry Detail", optional):
			fields.append(optional)
	return {row.name: row for row in frappe.get_all("Stock Entry Detail", filters={"name": ["in", names]}, fields=fields, limit_page_length=0)}
...
def get_voucher_headers(stock_ledger_rows):
	...
	for voucher_type, names in voucher_names.items():
		...
		try:
			rows = frappe.get_list(voucher_type, filters={"name": ["in", sorted(names)]}, fields=fields, limit_page_length=0)
...
def build_stock_entry_movement_row(group_rows, detail_map, header_map, item, conversion_map, compare_uom):
	...
	source = (detail and detail.get("s_warehouse")) or next((row.warehouse for row in group_rows if flt(row.actual_qty) < 0), None)
	destination = (detail and detail.get("t_warehouse")) or next((row.warehouse for row in group_rows if flt(row.actual_qty) > 0), None)
	...
	return make_output_row(
		sort_key="|".join(sorted(row.name for row in group_rows)), creation=min(row.creation for row in group_rows), posting_datetime=first.posting_datetime,
		movement_type=classify_stock_entry_movement(header.get("purpose"), source, destination), item_code=first.item_code,
		item_name=(detail and detail.get("item_name")) or item.get("item_name"), stock_uom=stock_uom,
		in_quantity=positive_qty or None, out_quantity=negative_qty or None, balance=None, compare_uom=compare_uom, conversion_factor=factor,
		source_warehouse=source, destination_warehouse=destination, voucher_type=first.voucher_type, voucher_no=first.voucher_no,
		voucher_detail_no=first.voucher_detail_no, purpose=clean_text(header.get("purpose") or header.get("title")),
		batch_no=batch_numbers[0] if len(batch_numbers) == 1 else "", batch_numbers=batch_numbers, remarks=clean_text(header.get("remarks") or header.get("remark")),
	)
...
def build_single_movement_row(sle, header, item, conversion_map, compare_uom):
	...
	return make_output_row(
		sort_key=sle.name, creation=sle.creation, posting_datetime=sle.posting_datetime,
		movement_type=classify_ledger_movement(sle.voucher_type, incoming, header), item_code=sle.item_code, item_name=item.get("item_name"), stock_uom=stock_uom,
		in_quantity=actual_qty if incoming else None, out_quantity=abs(actual_qty) if actual_qty < 0 else None, balance=None,
		compare_uom=compare_uom, conversion_factor=factor, source_warehouse=None if incoming else sle.warehouse, destination_warehouse=sle.warehouse if incoming else None,
		ledger_warehouse=sle.warehouse, reconciliation_balance=flt(sle.qty_after_transaction) if is_reconciliation else None,
		voucher_type=sle.voucher_type, voucher_no=sle.voucher_no, voucher_detail_no=sle.voucher_detail_no,
		purpose=clean_text(header.get("purpose") or header.get("return_against") or header.get("project") or header.get("title")),
		batch_no=sle.get("batch_no"), batch_numbers=[sle.get("batch_no")] if sle.get("batch_no") else [], remarks=clean_text(header.get("remarks") or header.get("remark")),
	)


def get_opening_balance(filters, warehouse_scope=None):
	opening_date = add_days(getdate(filters.from_date), -1)
	return flt(get_stock_balance(filters.item_code, filters.warehouse, posting_date=opening_date, posting_time="23:59:59.999999"))
...
def build_opening_balance_row(filters, *, item, conversion_map, opening_balance, opening_context=None):
	...
	opening_date = add_days(getdate(filters.from_date), -1)
	purpose, remarks, voucher_type, voucher_no = _("Opening stock before report period"), _("Balance as at {0} 23:59:59").format(opening_date), None, None
	if opening_context:
		purpose, voucher_type, voucher_no = _("Opening Stock Reconciliation"), "Stock Reconciliation", opening_context.get("voucher_no")
		remarks = _("Opening balance set by {0}").format(voucher_no)
	return make_output_row(
		sort_key="__opening_balance__", creation=f"{filters.from_date} 00:00:00", posting_datetime=f"{filters.from_date} 00:00:00",
		movement_type=_("Opening Balance"), item_code=filters.item_code, item_name=item.get("item_name"), stock_uom=stock_uom,
		in_quantity=None, out_quantity=None, balance=flt(opening_balance), compare_uom=compare_uom, conversion_factor=factor,
		source_warehouse=None, destination_warehouse=None, voucher_type=voucher_type, voucher_no=voucher_no, voucher_detail_no=None,
		purpose=purpose, batch_no=None, batch_numbers=[], remarks=remarks, is_opening_row=1,
	)
...
def apply_running_balances(rows, opening_balance=0):
	...
	for row in rows:
		if row.get("voucher_type") == "Stock Reconciliation" and row.get("reconciliation_balance") is not None:
			target = flt(row.get("reconciliation_balance"))
			adjustment = target - balance
			row["in_quantity"], row["out_quantity"] = (adjustment if adjustment > 0 else None), (abs(adjustment) if adjustment < 0 else None)
			warehouse = row.get("ledger_warehouse")
			row["source_warehouse"], row["destination_warehouse"] = (warehouse if adjustment < 0 else None), (warehouse if adjustment > 0 else None)
			row["movement_type"] = "Adjustment In" if adjustment > 0 else "Adjustment Out" if adjustment < 0 else "Stock Reconciliation"
			balance = target
		else:
			balance = balance - flt(row.get("out_quantity")) + flt(row.get("in_quantity"))
		row["balance"] = balance
		factor = row.get("_conversion_factor")
		row["compare_in_quantity"] = convert_quantity(row.get("in_quantity"), factor) if row.get("compare_uom") else None
		row["compare_out_quantity"] = convert_quantity(row.get("out_quantity"), factor) if row.get("compare_uom") else None
...
def apply_display_filters(rows, filters):
	...
	if filters.get("batch_no"):
		filtered = [row for row in filtered if filters.batch_no in (row.get("batch_numbers") or []) or row.get("batch_no") == filters.batch_no]
	return filtered


def make_output_row(**values):
	factor = values.pop("conversion_factor", None)
	in_quantity, out_quantity, balance = values.pop("in_quantity", None), values.pop("out_quantity", None), values.pop("balance", None)
...
def classify_stock_entry_movement(purpose, source_warehouse, destination_warehouse):
	...
	mapping = {"Material Issue": "Material Issue", "Material Receipt": "Material Receipt", "Manufacture": "Manufacture", "Repack": "Repack", "Send to Subcontractor": "Subcontract Transfer"}
...
def classify_ledger_movement(voucher_type, incoming, header=None):
	...
	return "Stock Reconciliation" if voucher_type == "Stock Reconciliation" else "Incoming" if incoming else "Outgoing"
...
def get_report_summary(rows):
	...
	warehouses = {warehouse for row in movement_rows for warehouse in (row.get("source_warehouse"), row.get("destination_warehouse")) if warehouse}
	sales = {(row.get("voucher_type"), row.get("voucher_no")) for row in movement_rows if row.get("movement_type") == "Sale" and row.get("voucher_no")}
	transfers = {(row.get("voucher_no"), row.get("voucher_detail_no"), row.get("item_code")) for row in movement_rows if row.get("movement_type") == "Internal Transfer"}
	...
	return [
		...
		{"value": len({row.get("item_code") for row in movement_rows if row.get("item_code")}), "label": _("Distinct Items"), "datatype": "Int", "indicator": "Blue"},
		...
		{"value": missing, "label": _("Missing UOM Conversions"), "datatype": "Int", "indicator": "Orange" if missing else "Green"},
	]
