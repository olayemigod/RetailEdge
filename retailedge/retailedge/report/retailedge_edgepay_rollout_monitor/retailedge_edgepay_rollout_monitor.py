...
def execute(filters=None):
	...
	pending_review = frappe.db.count(
		"RetailEdge EdgePay Payment Evidence", {"review_status": "Pending Review"}
	)
	draft_prepared = frappe.db.count(
		"RetailEdge EdgePay Payment Evidence",
		{"review_status": "Reviewed", "posting_status": "Draft Created"},
	)
	submitted_pe = frappe.db.count("RetailEdge EdgePay Payment Evidence", {"submission_status": "Submitted"})
	reconciliation_ready = frappe.db.count(
		"RetailEdge EdgePay Payment Evidence", {"reconciliation_status": "Ready"}
	)
	match_reviews = frappe.db.count(
		"RetailEdge EdgePay Payment Evidence", {"reconciliation_status": "Matched"}
	)
	confirmed_matches = frappe.db.count(
		"RetailEdge EdgePay Payment Evidence", {"reconciliation_status": "Reconciled"}
	)
	blocked_records = frappe.db.count(
	...
	stale_handoffs = frappe.db.count(
		"RetailEdge EdgePay Handoff Log",
		{"processing_status": "Pending", "creation": ["<=", stale_threshold]},
	)

	# 2. Payment Evidence not fully reconciled/blocked older than threshold
	stale_evidence = frappe.db.count(
		"RetailEdge EdgePay Payment Evidence",
		{"reconciliation_status": ["not in", ["Reconciled", "Blocked"]], "modified": ["<=", stale_threshold]},
	)
	...
	return [
		{
			...
			"value": summary_map.get(_("Pending Evidence Review"), 0),
			"datatype": "Int",
			"indicator": "Orange",
		},
		{
			"label": _("Draft Prepared"),
			"value": summary_map.get(_("Draft Payment Entries Prepared"), 0),
			"datatype": "Int",
			"indicator": "Orange",
		},
		{
			"label": _("Reconciliation Ready"),
			"value": summary_map.get(_("Reconciliation-Ready Records"), 0),
			"datatype": "Int",
			"indicator": "Orange",
		},
		{
			"label": _("Failed/Blocked"),
			"value": summary_map.get(_("Failed Handoffs"), 0)
			+ summary_map.get(_("Blocked/Exception Records"), 0),
			"datatype": "Int",
			"indicator": "Red",
		},
		{
			"label": _("Stale Records"),
			"value": summary_map.get(_("Stale Records"), 0),
			"datatype": "Int",
			"indicator": "Red",
		},
	]
