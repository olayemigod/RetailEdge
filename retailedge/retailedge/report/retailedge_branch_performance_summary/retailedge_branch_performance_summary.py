...
def get_columns():
	return [
		...
		{
			"label": _("Cashier Expenses"),
			"fieldname": "cashier_expenses",
			"fieldtype": "Currency",
			"width": 130,
		},
		{
			"label": _("Net Cash Expected"),
			"fieldname": "net_cash_expected",
			"fieldtype": "Currency",
			"width": 130,
		},
...
def get_report_summary(rows, message=None):
	if not rows:
		return [
			{
				"value": message or _("No matching records found for the selected filters."),
	...
	if has_ledger_expenses:
		summary.append(
			{
				"value": total_ledger_expenses,
				"label": _("Ledger Expenses"),
				"datatype": "Currency",
				"indicator": "Orange",
			}
		)
	summary.extend(
		[
			{
				"value": total_expected_cash,
				"label": _("Expected Cash"),
				"datatype": "Currency",
				"indicator": "Blue",
			},
			{
				"value": total_actual_cash,
				"label": _("Actual Cash"),
				"datatype": "Currency",
				"indicator": "Blue",
			},
			{
				...
				"label": _("Audit Variance"),
				"datatype": "Currency",
				"indicator": "Red" if total_variance else "Green",
			},
			{
				"value": total_absolute_variance,
				"label": _("Absolute Audit Variance"),
				"datatype": "Currency",
				"indicator": "Red" if total_absolute_variance else "Green",
			},
			{
				"value": total_issues,
				"label": _("Payment Issues"),
				"datatype": "Int",
				"indicator": "Orange" if total_issues else "Green",
			},
		]
	)
	return summary
