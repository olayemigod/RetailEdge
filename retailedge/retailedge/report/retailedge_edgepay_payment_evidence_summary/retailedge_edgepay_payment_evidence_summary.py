...
def execute(filters=None):
	...
	if filters.get("from_date") and filters.get("to_date"):
		db_filters["creation"] = [
			"between",
			[filters.get("from_date") + " 00:00:00", filters.get("to_date") + " 23:59:59"],
		]
	...
	evidences = frappe.get_all(
		...
		fields=[
			"name",
			"edgepay_handoff_event",
			"edgepay_payment_request",
			"source_doctype",
			"source_name",
			"amount",
			"currency",
			...
			"payment_entry",
			"linked_bank_transaction",
			"linked_bank_match_review",
...
def get_columns():
	return [
		...
		{
			"label": _("Handoff Event"),
			"fieldname": "edgepay_handoff_event",
			"fieldtype": "Link",
			"options": "EdgePay Status Handoff Event",
			"width": 170,
		},
		...
		{
			"label": _("Linked Bank Transaction"),
			"fieldname": "linked_bank_transaction",
			"fieldtype": "Link",
			"options": "Bank Transaction",
			"width": 170,
		},
		{
			"label": _("Linked Bank Match Review"),
			"fieldname": "linked_bank_match_review",
			"fieldtype": "Link",
			"options": "RetailEdge Bank Transaction Match",
			"width": 170,
		},
		{
			"label": _("Reconciliation Message"),
			"fieldname": "reconciliation_message",
...
def get_report_summary(rows):
	return [
		{"label": _("Total Evidence"), "value": len(rows), "datatype": "Int", "indicator": "Blue"},
		{
			...
			"value": sum(1 for r in rows if r.get("review_status") == "Pending Review"),
			"datatype": "Int",
			"indicator": "Orange",
		},
		{
			"label": _("Reviewed / Ready"),
			"value": sum(1 for r in rows if r.get("review_status") == "Reviewed"),
			"datatype": "Int",
			"indicator": "Green",
		},
		{
			"label": _("Submitted PE"),
			"value": sum(1 for r in rows if r.get("submission_status") == "Submitted"),
			"datatype": "Int",
			"indicator": "Blue",
		},
		{
			"label": _("Blocked / Exception"),
			"value": sum(1 for r in rows if r.get("reconciliation_status") in ("Blocked", "Exception")),
			"datatype": "Int",
			"indicator": "Red",
		},
	]
