		...
		if ev.edgepay_handoff_event:
			handoff_status = (
				frappe.db.get_value(
					...
					{"edgepay_event": ev.edgepay_handoff_event},
					"processing_status",
				)
				or "Processed"
			)
	...
	return [
		{
			...
			"fieldtype": "Link",
			"options": "EdgePay Payment Request",
			"width": 170,
		},
		{
			"label": _("Handoff Log"),
			"fieldname": "handoff_log",
			"fieldtype": "Link",
			"options": "RetailEdge EdgePay Handoff Log",
			"width": 170,
		},
		{
		...
		{
			"label": _("Bank Match Review"),
			"fieldname": "bank_match_review",
			"fieldtype": "Link",
...
def get_report_summary(rows):
	return [
		...
		{
			"label": _("Handoff Processed"),
			"value": sum(1 for r in rows if r.get("handoff_status") == "Processed"),
			"datatype": "Int",
			"indicator": "Green",
		},
		{
			"label": _("Evidence Reviewed"),
			"value": sum(1 for r in rows if r.get("review_status") == "Reviewed"),
			"datatype": "Int",
			"indicator": "Green",
		},
		{
			"label": _("Payment Entries Submitted"),
			"value": sum(1 for r in rows if r.get("submission_status") == "Submitted"),
			"datatype": "Int",
			"indicator": "Blue",
		},
		{
			"label": _("Reconciliation Confirmed"),
			"value": sum(1 for r in rows if r.get("reconciliation_status") in ("Matched", "Reconciled")),
			"datatype": "Int",
			"indicator": "Green",
		},
	]
