		...
		if (frm.doc.review_status === "Reviewed" && frm.doc.docstatus === 0) {
			if (!frm.doc.payment_entry) {
				frm.add_custom_button(__("Prepare Draft Payment Entry"), function () {
					frappe.call({
						...
						callback: function (r) {
							if (r.message && r.message.ok) {
								frappe.show_alert({
									message: __("Draft Payment Entry {0} prepared successfully.", [
				...
				frm.add_custom_button(__("Submit Payment Entry"), function () {
					frappe.confirm(
						__(
							"Are you sure you want to submit the linked Payment Entry {0}? This will post to the ledger.",
				...
				frappe.call({
					...
					callback: function (r) {
						...
						if (candidates.length === 0) {
							frappe.msgprint(
								__("No candidate Bank Transactions found matching this evidence.")
							);
							return;
						}
													...
													if (
														...
														frappe.show_alert({
															message: __(
																...
																[create_res.message.review_name]
															),
															indicator: "green",
														});
														frm.reload_doc();
											...
											if (confirm_res.message && confirm_res.message.ok) {
												...
												frappe.msgprint(
													confirm_res.message.message ||
														__("Failed to confirm match review.")
												);
											}
										},
									});
