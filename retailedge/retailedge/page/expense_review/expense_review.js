...
const EXPENSE_REVIEW_ASSET = "expense_review.bundle.js";
const PAGE_ROUTE = "expense-review";
const PAGE_TITLE = "Expense Review";
...
function renderLoadError(wrapper, error) {
	const errorDiv = document.createElement("div"); errorDiv.className = "retailedge-expense-review-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(EXPENSE_REVIEW_ASSET); if (typeof window.mountExpenseReviewPage !== "function") throw new Error("Expense Review bundle is unavailable.");
		bootLoading.remove(); const root = document.createElement("div"); root.className = "retailedge-expense-review-root"; page.body.append(root); await window.mountExpenseReviewPage(root);
