...
const BUSINESS_EXPENSE_ASSET = "business_expenses.bundle.js";
const PAGE_ROUTE = "business-expenses";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	loading.textContent = __("Loading Business Expenses...");
	wrapper.appendChild(loading);
	try {
		const page = frappe.ui.make_app_page({
			parent: wrapper,
			title: __("Business Expenses"),
			single_column: true,
		});
		wrapper.page = page;
		hideNativePageSidebar(wrapper);
		...
		await requireAsync(BUSINESS_EXPENSE_ASSET);
		if (typeof window.mountRetailEdgeBusinessExpenses !== "function") {
			throw new Error("Business Expenses bundle is unavailable.");
		}
		loading.remove();
