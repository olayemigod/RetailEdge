...
const CASH_MOVEMENT_ASSET = "cash_movement.bundle.js";
const PAGE_ROUTE = "cash-movement";
const PAGE_TITLE = "Cash Movement";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-cash-movement-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-cash-movement-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(CASH_MOVEMENT_ASSET);
		if (typeof window.mountCashMovement !== "function") throw new Error("Cash Movement bundle is unavailable.");
		bootLoading.remove();
