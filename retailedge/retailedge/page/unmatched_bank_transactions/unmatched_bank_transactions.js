...
const PAGE_ROUTE = "unmatched-bank-transactions";
const SURFACE_KEY = "unmatched-bank-transactions";
const PAGE_TITLE = "Unmatched Bank Transactions";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(REPORT_ASSET);
		if (typeof window.mountManagedReviewReport !== "function") throw new Error("Review workspace is unavailable.");
		loading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-managed-review-root";
		page.body.append(root);
		window.mountManagedReviewReport(root, SURFACE_KEY);
	} catch (error) {
		loading.remove();
		const block = document.createElement("div");
		block.className = "alert alert-danger p-6";
		block.textContent = window.retailedge?.userErrorMessage?.(error, __(PAGE_TITLE + " could not be loaded.")) || __(PAGE_TITLE + " could not be loaded.");
