...
const PAGE_ROUTE = "sales-invoice-register";
const REPORT_TYPE = "sales_invoice_register";
const PAGE_TITLE = "Sales Invoice Register";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-sales-report-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-sales-report-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountSalesReportingPage !== "function") throw new Error("Sales reporting bundle is unavailable.");
		bootLoading.remove();
