...
const PAGE_ASSET = "profitability_intelligence.bundle.js";
const PAGE_ROUTE = "profitability-intelligence";
const PAGE_TITLE = "Profitability Intelligence";
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-profitability-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountProfitabilityIntelligence !== "function") throw new Error("Profitability Intelligence bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div"); root.className = "retailedge-profitability-root"; page.body.append(root);
		await window.mountProfitabilityIntelligence(root);
