...
const PAGE_ROUTE = "expense-analysis";
const PAGE_TITLE = "Expense Analysis";
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-expense-analysis-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountExpenseRegister !== "function") throw new Error("Expense reporting bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-expense-analysis-root";
		page.body.append(root);
		await window.mountExpenseRegister(root, { reportType: "expense_analysis" });
	} catch (error) {
		bootLoading.remove();
