...
const INVENTORY_INTELLIGENCE_ASSET = "inventory_intelligence.bundle.js";
const PAGE_ROUTE = "inventory-intelligence";
const PAGE_TITLE = "Inventory Intelligence";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-inventory-intelligence-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-inventory-intelligence-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(INVENTORY_INTELLIGENCE_ASSET);
		if (typeof window.mountInventoryIntelligence !== "function") throw new Error("Inventory Intelligence bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-inventory-intelligence-root";
		page.body.append(root);
		await window.mountInventoryIntelligence(root);
		wrapper._retailedgeInventoryIntelligenceMounted = true;
	} catch (error) {
		bootLoading.remove();
