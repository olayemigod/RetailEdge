...
const CUSTOMER_360_ASSET = "customer_360.bundle.js";
const PAGE_ROUTE = "customer-360";
const PAGE_TITLE = "Customer 360";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-customer-360-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-customer-360-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(CUSTOMER_360_ASSET);
		if (typeof window.mountCustomer360 !== "function") throw new Error("Customer 360 bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-customer-360-root";
		page.body.append(root);
		await window.mountCustomer360(root);
		wrapper._retailedgeCustomer360Mounted = true;
	} catch (error) {
		bootLoading.remove();
