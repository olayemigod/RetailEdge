...
const SALES_QUALITY_ASSET = "sales_quality_intelligence.bundle.js";
const PAGE_ROUTE = "sales-quality-intelligence";
const PAGE_TITLE = "Discount & Sales Quality";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-sales-quality-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-sales-quality-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(SALES_QUALITY_ASSET);
		if (typeof window.mountSalesQualityIntelligence !== "function") throw new Error("Discount & Sales Quality bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-sales-quality-root";
		page.body.append(root);
		await window.mountSalesQualityIntelligence(root);
		wrapper._retailedgeSalesQualityMounted = true;
