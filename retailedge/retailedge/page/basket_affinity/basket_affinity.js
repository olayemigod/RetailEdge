...
const BASKET_AFFINITY_ASSET = "basket_affinity.bundle.js";
const PAGE_ROUTE = "basket-affinity";
const PAGE_TITLE = "Basket & Product Affinity";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-basket-affinity-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-basket-affinity-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(BASKET_AFFINITY_ASSET);
		if (typeof window.mountBasketAffinity !== "function") throw new Error("Basket & Product Affinity bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-basket-affinity-root";
		page.body.append(root);
		await window.mountBasketAffinity(root);
		wrapper._retailedgeBasketAffinityMounted = true;
