...
const INVENTORY_INSIGHTS_ASSET = "inventory_insights.bundle.js";
const PAGE_ROUTE = "inventory-transfer-opportunities";
const PAGE_TITLE = "Transfer Opportunities";
const INSIGHT_VIEW = "transfer-opportunities";

function requireAsync(assetName) {
	return new Promise((resolve, reject) => {
		let completed = false;
		const finish = () => { if (!completed) { completed = true; resolve(); } };
		const fail = (error) => { if (!completed) { completed = true; reject(error instanceof Error ? error : new Error(String(error || assetName))); } };
		try {
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-inventory-insight-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(INVENTORY_INSIGHTS_ASSET);
		if (typeof window.mountInventoryInsightView !== "function") throw new Error("Inventory insight bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-inventory-insight-root";
		page.body.append(root);
		await window.mountInventoryInsightView(root, { view: INSIGHT_VIEW });
	} catch (error) {
		bootLoading.remove();
