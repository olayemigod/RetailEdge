...
const PAGE_ASSET = "record_purchase.bundle.js";
const PAGE_ROUTE = "record-purchase";
const PAGE_TITLE = "Record Purchase";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountRetailEdgeRecordPurchase !== "function") throw new Error("Record Purchase bundle is unavailable.");
		loading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-record-purchase-root";
		page.body.append(root);
		wrapper._retailedgeTransactionEntryApp = await window.mountRetailEdgeRecordPurchase(root);
	} catch (error) {
		loading.remove();
		const node = document.createElement("div");
		node.className = "alert alert-danger p-6 text-center";
		node.textContent = error?.message || __("Record Purchase failed to load.");
...
frappe.pages[PAGE_ROUTE].on_page_show = function (wrapper) {
	hideNativePageSidebar(wrapper);
	window.dispatchEvent(new CustomEvent("retailedge-record-purchase-page-show"));
};
