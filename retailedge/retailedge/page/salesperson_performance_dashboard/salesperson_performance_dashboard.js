...
const DASHBOARD_ASSET = "salesperson_performance.bundle.js";
const PAGE_ROUTE = "salesperson-performance-dashboard";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-dashboard-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-dashboard-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	bootLoading.textContent = __("Loading Salesperson Performance Dashboard...");
	...
	try {
		...
		if (typeof window.mountSalespersonPerformanceDashboard !== "function") {
			throw new Error("Salesperson Performance dashboard bundle is unavailable.");
		}

		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-dashboard-root";
		page.body.append(root);
		await window.mountSalespersonPerformanceDashboard(root);
		wrapper._retailedgeDashboardMounted = true;
	} catch (error) {
		bootLoading.remove();
