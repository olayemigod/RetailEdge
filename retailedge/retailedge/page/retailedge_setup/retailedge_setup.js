...
const SETUP_ASSET = "retailedge_setup.bundle.js";
const PAGE_ROUTE = "retailedge-setup";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-setup-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-setup-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(SETUP_ASSET);
		if (typeof window.mountRetailEdgeSetup !== "function") throw new Error("RetailEdge Setup bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-setup-root";
		page.body.append(root);
		wrapper._retailedgeSetupApp = await window.mountRetailEdgeSetup(root);
	} catch (error) {
		bootLoading.remove();
...
frappe.pages[PAGE_ROUTE].on_page_show = function (wrapper) {
	hideNativePageSidebar(wrapper);
	window.dispatchEvent(new CustomEvent("retailedge-setup-page-show"));
};
