...
const CUSTOMER_INTELLIGENCE_ASSET = "customer_sales_intelligence.bundle.js";
const PAGE_ROUTE = "customer-sales-intelligence";
const PAGE_TITLE = "Customer & Sales Intelligence";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-customer-sales-intelligence-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-customer-sales-intelligence-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(CUSTOMER_INTELLIGENCE_ASSET);
		if (typeof window.mountCustomerSalesIntelligence !== "function") throw new Error("Customer & Sales Intelligence bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-customer-sales-intelligence-root";
		page.body.append(root);
		await window.mountCustomerSalesIntelligence(root);
		wrapper._retailedgeCustomerSalesIntelligenceMounted = true;
	} catch (error) {
		bootLoading.remove();
