...
const SALES_FORECAST_ASSET = "sales_forecast.bundle.js";
const PAGE_ROUTE = "sales-forecast";
const PAGE_TITLE = "Sales Forecast";
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-sales-forecast-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(SALES_FORECAST_ASSET);
		if (typeof window.mountSalesForecast !== "function") throw new Error("Sales Forecast bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-sales-forecast-root";
		page.body.append(root);
		await window.mountSalesForecast(root);
		wrapper._retailedgeSalesForecastMounted = true;
	} catch (error) {
		bootLoading.remove();
