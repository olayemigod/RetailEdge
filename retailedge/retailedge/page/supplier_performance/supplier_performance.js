...
const PAGE_ROUTE = "supplier-performance";
const REPORT_TYPE = "supplier_performance";
const PAGE_TITLE = "Supplier Performance";
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-purchase-report-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(PURCHASE_REPORTING_ASSET);
		if (typeof window.mountPurchaseReportingPage !== "function") throw new Error("Purchase reporting bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-purchase-report-root";
		page.body.append(root);
		await window.mountPurchaseReportingPage(root, { reportType: REPORT_TYPE });
	} catch (error) {
		bootLoading.remove();
