...
const PAGE_ASSET = "make_sale.bundle.js";
const PAGE_ROUTE = "make-sale";
const PAGE_TITLE = "Make Sale";
...
function renderLoadError(wrapper, error) {
	...
	node.className = "make-sale-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountRetailEdgeMakeSale !== "function") throw new Error("Make Sale bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-make-sale-root";
		page.body.append(root);
		wrapper._retailedgeMakeSaleApp = await window.mountRetailEdgeMakeSale(root);
	} catch (error) {
		bootLoading.remove();
...
frappe.pages[PAGE_ROUTE].on_page_show = function (wrapper) {
	hideNativePageSidebar(wrapper);
	window.dispatchEvent(new CustomEvent("retailedge-make-sale-page-show"));
};
