...
const PAGE_ROUTE = "cashier-expenses";
const PAGE_TITLE = "Cashier Expenses";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		root.className = "retailedge-cashier-expenses-root";
		page.body.append(root);
		wrapper._retailedgeVueApp = await window.mountExpenseRegister(root, { reportType: "cashier_expenses" });
	} catch (error) {
		bootLoading.remove();
