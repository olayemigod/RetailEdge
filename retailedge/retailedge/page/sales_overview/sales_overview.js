...
const DASHBOARD_ASSET = "sales_dashboard.bundle.js";
const PAGE_ROUTE = "sales-overview";
const PAGE_TITLE = "Sales Overview";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-sales-overview-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-sales-overview-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountSalesDashboard !== "function") throw new Error("Sales Overview bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-sales-overview-root";
		page.body.append(root);
		await window.mountSalesDashboard(root);
		wrapper._retailedgeSalesOverviewMounted = true;
	} catch (error) {
		bootLoading.remove();
