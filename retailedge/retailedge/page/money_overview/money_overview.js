...
const DASHBOARD_ASSET = "money_overview.bundle.js";
const PAGE_ROUTE = "money-overview";
const PAGE_TITLE = "Money Overview";
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-money-overview-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		if (typeof window.mountMoneyOverview !== "function") throw new Error("Money Overview bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div"); root.className = "retailedge-money-overview-root"; page.body.append(root);
		await window.mountMoneyOverview(root); wrapper._retailedgeMoneyOverviewMounted = true;
