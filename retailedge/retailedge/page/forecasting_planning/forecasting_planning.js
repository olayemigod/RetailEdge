...
const PLANNING_ASSET = "forecasting_planning.bundle.js";
const PAGE_ROUTE = "forecasting-planning";
const PAGE_TITLE = "Forecasting & Planning";
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-forecasting-planning-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(PLANNING_ASSET);
		if (typeof window.mountForecastingPlanning !== "function") throw new Error("Forecasting & Planning bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-forecasting-planning-root";
		page.body.append(root);
		await window.mountForecastingPlanning(root);
		wrapper._retailedgeForecastingPlanningMounted = true;
	} catch (error) {
		bootLoading.remove();
