...
function renderLoadError(wrapper, error) {
	...
	node.className = "document-output-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(OUTPUT_ASSET);
		if (typeof window.mountRetailEdgeDocumentOutputSharing !== "function") throw new Error("Document Output & Sharing bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-document-output-root";
		page.body.append(root);
		wrapper._retailedgeDocumentOutputApp = await window.mountRetailEdgeDocumentOutputSharing(root);
	} catch (error) {
		bootLoading.remove();
...
frappe.pages[PAGE_ROUTE].on_page_show = function (wrapper) {
	hideNativePageSidebar(wrapper);
	window.dispatchEvent(new CustomEvent("retailedge-document-output-page-show"));
};
