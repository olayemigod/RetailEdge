...
const OPERATING_CONTEXT_ASSET = "operating_context.bundle.js";
const PAGE_ROUTE = "operating-context";
const PAGE_TITLE = "Operating Context";
...
function renderLoadError(wrapper, error) {
	const existing = wrapper.querySelector?.(".retailedge-operating-context-load-error");
	if (existing) existing.remove();
	const errorDiv = document.createElement("div");
	errorDiv.className = "retailedge-operating-context-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	try {
		...
		await requireAsync(OPERATING_CONTEXT_ASSET);
		if (typeof window.mountRetailEdgeOperatingContext !== "function") throw new Error("Operating Context bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div");
		root.className = "retailedge-operating-context-root";
		page.body.append(root);
		wrapper._retailedgeOperatingContextApp = await window.mountRetailEdgeOperatingContext(root);
	} catch (error) {
		bootLoading.remove();
...
frappe.pages[PAGE_ROUTE].on_page_show = function (wrapper) {
	hideNativePageSidebar(wrapper);
	window.dispatchEvent(new CustomEvent("retailedge-operating-context-page-show"));
};
