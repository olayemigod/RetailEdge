...
const DASHBOARD_ASSET = "expense_dashboard.bundle.js";
const PAGE_ROUTE = "expense-overview";
const PAGE_TITLE = "Expenses Dashboard";
...
function requireAsync(assetName) {
	return new Promise((resolve, reject) => {
		...
		try { const pending = frappe.require(assetName, finish); if (pending && typeof pending.then === "function") pending.then(finish).catch(fail); }
...
function renderLoadError(wrapper, error) {
	...
	errorDiv.className = "retailedge-expense-dashboard-load-error alert alert-danger p-6 text-center";
...
frappe.pages[PAGE_ROUTE].on_page_load = async function (wrapper) {
	...
	bootLoading.textContent = __(`Loading ${PAGE_TITLE}...`); wrapper.appendChild(bootLoading);
	try {
		...
		if (typeof window.mountExpenseDashboard !== "function") throw new Error("Expenses Dashboard bundle is unavailable.");
		bootLoading.remove();
		const root = document.createElement("div"); root.className = "retailedge-expense-dashboard-root"; page.body.append(root);
		await window.mountExpenseDashboard(root); wrapper._retailedgeExpenseDashboardMounted = true;
