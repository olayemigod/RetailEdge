...
def _patched_candidate_document_key(original, row):
	...
	if document_type != "Payment Entry" or not document_name or not _is_submitted_internal_transfer(document_name):
		return original(row)
	...
	if direction in {"Inflow", "Outflow"} and payment_account:
		return ("Payment Entry", document_name, direction, payment_account)
	bank_transaction = row.get("bank_transaction")
	key = _payment_entry_leg_key(document_name, bank_transaction) if bank_transaction else None
	return key if key and len(key) > 2 else original(row)
...
def install_internal_transfer_bank_leg_identity():
	...
	original_payment_entry_confirmed = matching.payment_entry_has_active_confirmed_bank_match
	...
	def payment_entry_has_active_confirmed_bank_match(payment_entry):
		...
		return original_payment_entry_confirmed(payment_entry)
	...
	def candidate_document_has_active_confirmed_bank_match(document_type, document_name):
		...
		return original_candidate_confirmed(document_type, document_name)
	...
	def active_review_match_for_candidate(document_type, document_name):
		...
		return original_active_review(document_type, document_name)
	...
	def find_active_candidate_review_match(suggested_document_type, suggested_document):
		...
		return original_find_active_candidate_review(suggested_document_type, suggested_document)
	...
	def get_first_active_confirmed_conflict(doc):
		...
		if payment_entry and _is_submitted_internal_transfer(payment_entry):
			...
			return None
		return original_first_confirmed_conflict(doc)
